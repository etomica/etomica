...
public class SimGlass extends Simulation {
    ...
    public SimGlass(int D, int nA, int nB, double density, double temperature, boolean doSwap, PotentialChoice pc, double tStep, int[] randSeeds) {
        ...
        if (potentialChoice == PotentialChoice.LJ) { //3D KA-80-20; 2D KA-65-35
            ...
            // start out with A-A core diameter at only 50% so our config has no overlap
            chs = 50;
        ...
        if (potentialChoice == PotentialChoice.HS) {
            // increase our A-A hard core diameter until we have an overlap
    ...
    public void initConfig() {
        ...
        // we were unable to create the configuration without overlaps, so run the simulation
        // now and increase the diameter until we reach 1.
        while (chs < 100) {
            integrator.reset();
