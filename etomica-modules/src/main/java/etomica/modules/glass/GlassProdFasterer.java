...
public class GlassProdFasterer {
    public static void main(String[] args) {
        ...
        SimGlassFasterer sim = new SimGlassFasterer(params.D, params.nA, params.nB, params.density, params.temperature, params.doSwap, params.potential, params.tStep, params.randomSeeds);
        ...
        MeterEnergyFromIntegratorFasterer meterE = new MeterEnergyFromIntegratorFasterer(sim.integrator);
        DataPumpListener pumpE = new DataPumpListener(meterE, accE, 10);
        if (sim.potentialChoice != SimGlassFasterer.PotentialChoice.HS && temperature0 == params.temperature) {
...
//            System.out.println("actual KE => "+newNewKE/numAtoms);
        ...
        boolean doPxyAutocor = params.doPxyAutocor && sim.potentialChoice != SimGlassFasterer.PotentialChoice.HS;
        if (doPxyAutocor) {
        ...
        atomSignalPacking.setAtomTypeFactor(sim.speciesB.getLeafType(), sim.potentialChoice == SimGlassFasterer.PotentialChoice.HS ? vB : 0);
        ...
        try {
            if (doPxyAutocor) {
                GlassProdFasterer.writeDataToFile(dpxyAutocor, filenamePxyAC);
            }
            GlassProdFasterer.writeDataToFile(dsCorMSD, "MSDcor.dat");
            GlassProdFasterer.writeDataToFile(dsCorP, "Pcor.dat");
            GlassProdFasterer.writeDataToFile(dsMSDcorP, "MSDcorP.dat");
            for (int i = 0; i < configStorageMSD.getLastConfigIndex() - 1; i++) {
                meterGs.setConfigIndex(i);
                GlassProdFasterer.writeDataToFile(meterGs, "Gs_t" + i + ".dat");
                meterGsA.setConfigIndex(i);
                GlassProdFasterer.writeDataToFile(meterGsA, "GsA_t" + i + ".dat");
                meterGsB.setConfigIndex(i);
                GlassProdFasterer.writeDataToFile(meterGsB, "GsB_t" + i + ".dat");
            }
            for (int i = 0; i < accSFacMobility.length; i++) {
                if (accSFacMobility[i].getSampleCount() < 2) continue;
                GlassProdFasterer.writeDataToFile(accSFacMobility[i], "sfacMobility" + i + ".dat");
                GlassProdFasterer.writeDataToFile(accSFacMotion[i], "sfacMotionx" + i + ".dat");
            }
            double fac = L / (2 * Math.PI);
            int[] foo = new int[mobilityMap.length];
            for (int j = 0; j < mobilityMap.length; j++) {
                String packing = sim.potentialChoice == SimGlassFasterer.PotentialChoice.HS ? "Packing" : "DensityA";
                ...
                GlassProdFasterer.writeDataToFile(m, "sfacMobilityCor_" + label);
                m = sfcDensityCor.makeMeter(mobilityMap[j]);
                GlassProdFasterer.writeDataToFile(m, "sfacDensityCor_" + label);
                m = sfcPackingCor.makeMeter(mobilityMap[j]);
                GlassProdFasterer.writeDataToFile(m, "sfac" + packing + "Cor_" + label);
                m = sfcStress2Cor.makeMeter(mobilityMap[j]);
                GlassProdFasterer.writeDataToFile(m, "sfacStressCor_" + label);
                m = sfcKECor.makeMeter(mobilityMap[j]);
                GlassProdFasterer.writeDataToFile(m, "sfacKineticCor_" + label);
                DataSourceCorrelation.Meter mm = dsCorSFacDensityMobility.makeMeter(j);
                GlassProdFasterer.writeDataToFile(mm, "sfacDensityMobilityCor_" + label);
                mm = dsCorSFacPackingMobility.makeMeter(j);
                GlassProdFasterer.writeDataToFile(mm, "sfac" + packing + "MobilityCor_" + label);
                mm = dsCorSFacPackingDensity.makeMeter(j);
                GlassProdFasterer.writeDataToFile(mm, "sfac" + packing + "DensityCor_" + label);
                mm = dsCorSFacStress2Mobility.makeMeter(j);
                GlassProdFasterer.writeDataToFile(mm, "sfacStressMobilityCor_" + label);
                mm = dsCorSFacKEMobility.makeMeter(j);
                GlassProdFasterer.writeDataToFile(mm, "sfacKineticMobilityCor_" + label);
            }
            ...
            for (int j = 0; j < motionMap.length; j++) {
                ...
                GlassProdFasterer.writeDataToFile(m, "sfacMotionCor_" + label);
            }

            GlassProdFasterer.writeDataToFile(meterCorrelationSelf, "corSelf.dat");
            GlassProdFasterer.writeDataToFile(meterCorrelationSelfMagA, "corSelfMagA.dat");
            GlassProdFasterer.writeDataToFile(meterCorrelationSelfMagB, "corSelfMagB.dat");
            for (int i = 0; i < correlationSelf2.getNumDt(); i++) {
                CorrelationSelf2.MeterCorrelationSelf2 m = correlationSelf2.makeMeter(i);
                GlassProdFasterer.writeDataToFile(m, "corRSelf_t" + i + ".dat");
            }

            GlassProdFasterer.writeDataToFile(meterFs, filenameFs);
            GlassProdFasterer.writeCombinedDataToFile(new IDataSource[]{meterPerc, meterPerc3}, filenamePerc);
            GlassProdFasterer.writeCombinedDataToFile(new IDataSource[]{meterPerc.makeImmFractionSource(),
                    meterPerc3.makeImmFractionSource()}, filenameImmFrac);
            GlassProdFasterer.writeCombinedDataToFile(new IDataSource[]{meterPerc.makeImmFractionSource(sim.speciesA.getLeafType()),
                    meterPerc3.makeImmFractionSource(sim.speciesA.getLeafType())}, filenameImmFracA);
            GlassProdFasterer.writeCombinedDataToFile(new IDataSource[]{meterPerc.makeImmFractionSource(sim.speciesB.getLeafType()),
                    meterPerc3.makeImmFractionSource(sim.speciesB.getLeafType())}, filenameImmFracB);
            GlassProdFasterer.writeDataToFile(meterPerc.makePerclationByImmFracSource(), filenameImmFracPerc);
            GlassProdFasterer.writeDataToFile(accPerc0, filenamePerc0);
            GlassProdFasterer.writeDataToFile(meterQ4, "Q4" + fileTag + ".out");
            GlassProdFasterer.writeDataToFile(meterL, filenameL);
            GlassProdFasterer.writeCombinedDataToFile(new IDataSource[]{meterPerc.makeChi4Source(), meterPerc3.makeChi4Source()}, "chi4Star" + fileTag + ".out");
            GlassProdFasterer.writeDataToFile(meterQ4.makeChi4Meter(), "chi4" + fileTag + ".out");
            GlassProdFasterer.writeDataToFile(meterAlpha2, filenameAlpha2);
            GlassProdFasterer.writeDataToFile(meterSFac, filenameSq);

            GlassProdFasterer.writeDataToFile(meterMSD, meterMSD.errData, filenameMSD);
            GlassProdFasterer.writeDataToFile(meterMSDA, meterMSDA.errData, filenameMSDA);
            GlassProdFasterer.writeDataToFile(meterMSDB, meterMSDB.errData, filenameMSDB);
            GlassProdFasterer.writeDataToFile(meterVAC, meterVAC.errData, filenameVAC);
            GlassProdFasterer.writeDataToFile(pTensorAccumVisc, pTensorAccumVisc.errData, filenameVisc);
        } catch (IOException e) {
    ...
    public static class SimParams extends SimGlassFasterer.GlassParams {
