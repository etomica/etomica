...
public class P2SquareWellBonded extends P2SquareWell {
    ...
    public P2SquareWellBonded(Space space, AtomLeafAgentManager<IAtom[]> aam, double coreDiameter, double lambda, double epsilon) {
...
//		if(!Double.isInfinite(time)) System.out.println("Collision time: "+time+" for "+atoms.toString());
    ...
    public void bump(IAtomList pair, double falseTime) {
        ...
        if (areBonded(atom0, atom1)) {        //atoms are bonded to each
            ...
            if (canBond) {
                int maxRingBonds = 20;
                IAtom[] aNbrs = agentManager.getAgent(atom0);
                if (aNbrs.length == 3) {
                    // cross linker
