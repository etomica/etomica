...
import etomica.modules.ensembles.LJMCFasterer;
...
public class StatisticsMCGraphicFasterer extends SimulationGraphic {

    private final static String APP_NAME = "Statistics";
    private final static int REPAINT_INTERVAL = 100;
    protected final LJMCFasterer sim;

    protected boolean volumeChanges = false;
    protected boolean constMu = false;

    public StatisticsMCGraphicFasterer(final LJMCFasterer simulation, int moduleNum) {
        ...
        MeterWidomInsertionFasterer meterWidom = new MeterWidomInsertionFasterer(sim.box, sim.getRandom(), sim.integrator.getPotentialCompute(), sim.integrator.getTemperature());
        ...
        AccumulatorFactory muFactory = new AccumulatorFactory() {
            ...
            public AccumulatorAverageFixed makeAccumulator() {
                return new AccumulatorMimicMuFasterer(sim.integrator);
            }

            public DataProcessor makeDataProcessor() {
                return new DataProcessorMuFasterer(sim.integrator);
            }
        };
        if (moduleNum == 1) {
...
//        widomAvg.addDataSink(accMu);
    ...
    protected HistoryPlotBits makeMuHistoryPlot(ArrayList<DataPump> dataStreamPumps, DataSourceCountSteps timeCounter, JPanel historyPanel, DataPumpListener pump, String name) {
        ...
        AccumulatorMimicMuFasterer accMu = new AccumulatorMimicMuFasterer(sim.integrator);
...
//        paneLog.setPreferredSize(paneSize);
    ...
    public static void main(String[] args) {
        ...
        StatisticsMCGraphicFasterer ljmcGraphic = new StatisticsMCGraphicFasterer(new LJMCFasterer(sp), moduleNum);
