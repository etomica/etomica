...
public class OsmosisFasterer extends SimulationGraphic {
	...
	public MeterOsmoticPressureFasterer osmosisPMeter;
	public MeterLocalMoleFraction moleFractionRight, moleFractionLeft;
	public OsmosisSimFasterer sim;
	private DeviceThermoSlider temperatureSelect;

	public OsmosisFasterer(OsmosisSimFasterer simulation) {
		...
		if (sim.getSpace() instanceof Space2D) {
			...
			((DisplayBoxCanvasG3DSys) displayBox.canvas).addPlane(plane);
		...
		displayCycles = new DisplayTimerFasterer(sim.integrator);
		displayCycles.setLabel("Cycle Time");
		displayCycles.setPrecision(6);

		// Right side of membrane osmotic
		osmosisPMeter = new MeterOsmoticPressureFasterer(new P1HardBoundary[]{sim.boundaryHardA, sim.boundaryHardB}, sim.integrator);
		osmosisPMeter.setCollisionRadius(sim.boundaryHardA.getCollisionRadius());
...
//        leftMFBox.setLabelType(LabelType.BORDER);
		...
		if (sim.getSpace() instanceof Space3D) { // 3D
			...
			((DisplayBoxCanvasG3DSys) displayBox.canvas).setBoundaryFrameColor(Color.BLACK);
			((DisplayBoxCanvasG3DSys) displayBox.canvas).setPlaneColor(Color.GREEN);
		}
	...
	public static void main(String[] args) {
		...
		OsmosisSimFasterer sim = null;

		Space sp = Space3D.getInstance();
		sim = new OsmosisSimFasterer(sp);

		sim.getController().setSleepPeriod(1);
		sim.getController().addActivity(new ActivityIntegrate(sim.integrator));

		OsmosisFasterer osmosis = new OsmosisFasterer(sim);
