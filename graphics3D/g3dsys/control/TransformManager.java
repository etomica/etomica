...
public class TransformManager {
  ...
  final Point3f fixedRotationCenter = Point3f.new3(0, 0, 0);
  ...
  synchronized void rotateXRadians(float angleRadians) {
    ...
    matrixRotate.mul2(matrixTemp3, matrixRotate);
  }
  ...
  void rotateTo(float x, float y, float z, float degrees) {
    ...
    if (degrees < .01 && degrees > -.01) {
      ...
      axisangleT.set4(x, y, z, degrees * radiansPerDegree);
      matrixRotate.setAA(axisangleT);
    }
  }
  ...
  private final Vector3f perspectiveOffset = Vector3f.new3(0, 0, 0);
  ...
  synchronized void finalizeTransformParameters() {
    ...
    if(windowCentered) {
      matrixTransform.transform2(rotationCenterDefault, pointT);
      matrixTransform.transform2(fixedRotationCenter, pointT2);
      perspectiveOffset.sub2(pointT, pointT2);
    }
  ...
  synchronized private void calcTransformMatrix() {
    ...
    matrixTemp.setTranslation(vectorTemp);
    if (axesOrientationRasmol)
        matrixTemp.mul(-1); // make all z positive
    matrixTransform.sub(matrixTemp);
    // now scale to screen coordinates
    matrixTemp.setIdentity();
    matrixTemp.mul(scalePixelsPerAngstrom);
    matrixTemp.m33 = 1.0f;
    if (!axesOrientationRasmol) {
  ...
  void unTransformPoint(Point3i screenPt, Point3f coordPt) {
    ...
    m.invertM(matrixTransform);
    m.transform2(pt, coordPt);
  }
  ...
  private void calcSlabAndDepthValues() {
    ...
    if (slabEnabled) {
      ...
      slabValue = (int)(((100 - slabPercentSetting) * 2 * radius / 100)
          + cameraDistance);
      depthValue = (int)(((100 - depthPercentSetting) * 2 * radius / 100)
          + cameraDistance);
    }
  }
