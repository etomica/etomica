...
import etomica.integrator.IntegratorRigidIterativeFasterer;
import etomica.lattice.LatticeCubicFcc;
import etomica.molecule.OrientationCalcAtomLinearFasterer;
...
public class DipoleBoxFasterer extends Simulation {

    public final IntegratorRigidIterativeFasterer integrator;
    public final Box box;

    public DipoleBoxFasterer(Space space, int nAtoms, double dt) {
        super(space);
        ...
        integrator = new IntegratorRigidIterativeFasterer(getSpeciesManager(), random, potentialMaster, dt, 1, box);
        integrator.setTemperature(1);
        integrator.setIsothermal(false);
        integrator.printInterval = 100;
        integrator.setMaxIterations(maxIterations);
        OrientationCalcAtomLinearFasterer calcer = new OrientationCalcAtomLinearFasterer();
    ...
    public static void main(String[] args) {
        ...
        double dt = 0.01;
        if (args.length == 0) {
            DipoleBoxFasterer sim = new DipoleBoxFasterer(space, nAtoms, dt);
    ...
    public static class Applet extends javax.swing.JApplet {
        ...
        public void init() {
            Space space = Space3D.getInstance();
            DipoleBoxFasterer sim = new DipoleBoxFasterer(space, 864, 0.01);
            sim.getController().setSleepPeriod(10);
            SimulationGraphic graphic = new SimulationGraphic(sim, "Rigid", 1);
            graphic.getDisplayBox(sim.box).setPixelUnit(new Pixel(30));
