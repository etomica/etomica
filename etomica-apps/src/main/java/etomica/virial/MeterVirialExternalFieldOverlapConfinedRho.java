    		      ...
    		      //for (int i=0; i <= wallPosition.length; i++){
    	    		...
    	    		for(int j=0; j<cList.size()-1; j++){
    	    			...
    	    			for(byte gm1 :gm1List){
    	    				...
    	    				if (atoms.get(gm1).getPosition().getX(2) < wallPosition + 0.5 || wallPosition + walldistance - 0.5 < atoms.get(gm1).getPosition().getX(2)){
    	    					g2 *= 0;
    	    				}    	   
    	    				else if (atoms.get(gm1).getPosition().getX(2) < wallPosition + 0.5 + lambdaWF){
    	    					g2 *= Math.exp(epsilonWF / temperature); 
    	    				}
    	    				if (atoms.get(gm1).getPosition().getX(2) >  wallPosition + walldistance - 0.5 - lambdaWF){
    	    					g2 *= Math.exp(epsilonWF / temperature); 
    	    				}
