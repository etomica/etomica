...
public class VirialEmulNonAdditive {
    ...
    public static void main(String[] args) {
        ...
        if (isCommandline) {
            ...
            params.template2 = "2body_template.in";
            params.template3 = "3body_template.in";
        }
        ...
        String template2 = params.template2;
        String template3 = params.template3;
        ...
        System.out.println("Emul overlap sampling B"+nPoints+"NonAdd at T="+temperatureK+ " K");
        ...
        multiDiagrams.setDoMinimalMulti(true);
        multiDiagrams.setDoMultiFromPair(true);
        multiDiagrams.setDoReeHoover(true);
        multiDiagrams.setDoShortcut(true);
        ClusterSum fullTargetCluster = multiDiagrams.makeVirialCluster(fTarget, f3Target, false);
        ...
        rigidDiagrams.setAllPermutations(false);
        ClusterSum refCluster = rigidDiagrams.makeVirialCluster(fRef);


        ClusterAbstract targetCluster = fullTargetCluster;
        ClusterAbstract[] targetDiagrams = multiDiagrams.makeSingleVirialClustersMulti((ClusterSumMultibody)fullTargetCluster, fTarget, f3Target);
        ...
        int[] mfeTargetDiagramNumbers = new int[targetDiagrams.length];
        int[] meTargetDiagramNumbers = new int[targetDiagrams.length];
        System.out.println("individual clusters:");
        Set<Graph> singleGraphs = multiDiagrams.getMSMCGraphs(true, true);
        int iGraph = 0;
        DeleteEdge edgeDeleter = new DeleteEdge();
        DeleteEdgeParameters edM = new DeleteEdgeParameters(multiDiagrams.mmBond);
        DeleteEdgeParameters edm = new DeleteEdgeParameters(multiDiagrams.mBond);
        DeleteEdgeParameters edF = new DeleteEdgeParameters(multiDiagrams.fBond);
        for (Graph g : singleGraphs) {
            if (g.edgeCount() == nPoints*(nPoints-1)/2) {
                if (VirialDiagrams.graphHasEdgeColor(g, multiDiagrams.mBond) || VirialDiagrams.graphHasEdgeColor(g, multiDiagrams.mmBond)) {
                    System.out.print(" ("+g.coefficient()+") "+g.nodeCount()+"M");
                    targetDiagramNumbers[iGraph] = -g.nodeCount();
                }
                else {
                    throw new RuntimeException("fully-connected non-multi diagram?");
                }
            }
            else {
                String gnStr = g.getStore().toNumberString();
                targetDiagramNumbers[iGraph] = Integer.parseInt(gnStr);
                Graph gOnlyFE = edgeDeleter.apply(edgeDeleter.apply(g, edm), edM);
                gnStr += "m"+gOnlyFE.getStore().toNumberString();
                mfeTargetDiagramNumbers[iGraph] = Integer.parseInt(gOnlyFE.getStore().toNumberString());
                Graph gOnlyE = edgeDeleter.apply(gOnlyFE, edF);
                if (gOnlyE.edgeCount() > 0) {
                    gnStr += "f"+gOnlyE.getStore().toNumberString();
                    meTargetDiagramNumbers[iGraph] = Integer.parseInt(gOnlyE.getStore().toNumberString());
                }
...
//                }
        ...
        for (int i=0; i<targetDiagrams.length; i++) {
            if (targetDiagramNumbers[i]<0) {
                ...
                if (mfeTargetDiagramNumbers[i]<0) {
                    System.out.print("diagram "+targetDiagramNumbers[i]+"rm"+(-mfeTargetDiagramNumbers[i])+" ");
                }
                else {
                    String gnString = targetDiagramNumbers[i]+"m"+mfeTargetDiagramNumbers[i];
                    if (meTargetDiagramNumbers[i] > 0) {
                        gnString += "f"+meTargetDiagramNumbers[i];
                    }
                    System.out.print("diagram "+gnString+" ");
                }
            }
