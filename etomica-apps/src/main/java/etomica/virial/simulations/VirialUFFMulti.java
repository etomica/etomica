...
public class VirialUFFMulti {
    ...
    public static void main(String[] args) {
        ...
        if (isCommandline) {
            ...
            params.level = VirialCO2H2OSC.Level.SEMICLASSICAL_FH;
        ...
        final VirialCO2H2OSC.Level level = params.level;
        final double refFrac = params.refFrac;
        final VirialCO2H2OSC.Nonadditive nonAdditive = nPoints < 3 ? VirialCO2H2OSC.Nonadditive.NONE : params.nonAdditive;
        final boolean useSZ = nonAdditive != VirialCO2H2OSC.Nonadditive.NONE && params.useSZ && nTypes[1]==nPoints;
        ...
        if (nonAdditive != VirialCO2H2OSC.Nonadditive.NONE) {
            if (nonAdditive == VirialCO2H2OSC.Nonadditive.DISPERSION) {
        ...
        if (level == VirialCO2H2OSC.Level.SEMICLASSICAL_FH) {
            System.out.println("Quadratic Feymann-Hibbs effective potential employed.");
        }
        if (level == VirialCO2H2OSC.Level.SEMICLASSICAL_TI) {
        ...
        MayerHardSphere fRef = new MayerHardSphere(sigmaHSRef); // Not required

        //Make species and make potential  till 119.
        ...
        P2CO2Hellmann p2cCO2 = new P2CO2Hellmann(space, P2CO2Hellmann.Parameters.B); //NR
        IPotential2 p2aCO2 = level == VirialCO2H2OSC.Level.CLASSICAL ? null : (level == VirialCO2H2OSC.Level.SEMICLASSICAL_FH ? p2cCO2.makeSemiclassical(temperature) : new P2SemiclassicalAtomic(space, p2cCO2, temperature));

        P2CO2H2OWheatley p2cCO2H2O = new P2CO2H2OWheatley(space);
        IPotential2 p2aCO2H2O = level == VirialCO2H2OSC.Level.CLASSICAL ? null : (level == VirialCO2H2OSC.Level.SEMICLASSICAL_FH ? p2cCO2H2O.makeSemiclassical(temperature) : p2cCO2H2O.makeSemiclassicalTI(temperature));

        P2WaterSzalewicz.P2WaterSzalewicz2 p2cH2O = P2WaterSzalewicz.make2Body();
        IPotential2 p2aH2O = level == VirialCO2H2OSC.Level.CLASSICAL ? null : (level == VirialCO2H2OSC.Level.SEMICLASSICAL_FH ? p2cH2O.makeSemiclassical(temperature) : new P2SemiclassicalAtomic(space, p2cH2O, temperature));

        P2MolecularMonatomic p2H2O = new P2MolecularMonatomic(space, level== VirialCO2H2OSC.Level.CLASSICAL ? p2cH2O : p2aH2O);
        MayerGeneralAtomic fCO2 = new MayerGeneralAtomic(space, level== VirialCO2H2OSC.Level.CLASSICAL ? p2cCO2 : p2aCO2);
        MayerGeneralAtomic fCO2H2O = new MayerGeneralAtomic(space, level== VirialCO2H2OSC.Level.CLASSICAL ? p2cCO2H2O : p2aCO2H2O); //TA
        MayerGeneral fH2O = new MayerGeneral(p2H2O);//TA
        MayerFunction[][] allF = new MayerFunction[][]{{fCO2,fCO2H2O},{fCO2H2O,fH2O}};//TA
        ...
        if (nonAdditive != VirialCO2H2OSC.Nonadditive.NONE) {
            if (useSZ && nPoints == nTypes[1]) {
                P2WaterSzalewicz.Component comp = nonAdditive == VirialCO2H2OSC.Nonadditive.FULL ? P2WaterSzalewicz.Component.NON_PAIR : P2WaterSzalewicz.Component.THREE_BODY;
                P2WaterSzalewicz.P2WaterSzalewicz2 p23cH2O = P2WaterSzalewicz.make2Body(comp);
                IPotential2 p23aH2O = level == VirialCO2H2OSC.Level.CLASSICAL ? null : (level == VirialCO2H2OSC.Level.SEMICLASSICAL_FH ? p23cH2O.makeSemiclassical(temperature) : new P2SemiclassicalAtomic(space, p23cH2O, temperature));
                P2MolecularMonatomic p23H2O = new P2MolecularMonatomic(space, level== VirialCO2H2OSC.Level.CLASSICAL ? p23cH2O : p23aH2O);

                P2WaterSzalewicz.P2WaterSzalewicz3 p3cH2O = P2WaterSzalewicz.make3Body(comp);
                IPotential3 p3aH2O = level == VirialCO2H2OSC.Level.CLASSICAL ? null : (level == VirialCO2H2OSC.Level.SEMICLASSICAL_FH ? p3cH2O.makeSemiclassical(temperature) : null);
                P3MolecularMonatomic p3H2O = new P3MolecularMonatomic(space, level== VirialCO2H2OSC.Level.CLASSICAL ? p3cH2O : p3aH2O);
                ...
                if (nonAdditive == VirialCO2H2OSC.Nonadditive.FULL) {
                ...
                allFNA[1][1][1] = f3;
                targetCluster = new ClusterWheatleyMultibodyMix(nPoints, nTypes, allF, allFNA, 1e-12, true);
                if (nonAdditive == VirialCO2H2OSC.Nonadditive.FULL && nTypes[1] > 0) {
                    ((ClusterWheatleyMultibodyMix)targetCluster).setDoCaching(false);
        ...
        if (nonAdditive != VirialCO2H2OSC.Nonadditive.NONE && !useSZ) {
        ...
        if (level == VirialCO2H2OSC.Level.SEMICLASSICAL_TI && false) {
            ...
            ((P2SemiclassicalAtomic)p2aCO2).setAtomInfo(speciesCO2.getLeafType(), new P2SemiclassicalAtomic.AtomInfo() {
...
//            simGraphic.makeAndDisplayFrame();
        ...
        if (isCommandline) {
            ...
            refFileName = "refpref"+nPoints+"_"+(nonAdditive== VirialCO2H2OSC.Nonadditive.NONE?"2":("3"+(nonAdditive== VirialCO2H2OSC.Nonadditive.DISPERSION?"a":"ai")))+(useSZ?"_sz":"")+"_"+tempString;
            refFileName += level == VirialCO2H2OSC.Level.CLASSICAL ? "_c" : (level == VirialCO2H2OSC.Level.SEMICLASSICAL_FH ? "_fh" : "_ti");
            refFileName += "C";
        }
    ...
    public static class VirialCO2SCParam extends ParameterBase {
        ...
        public VirialCO2H2OSC.Level level = VirialCO2H2OSC.Level.CLASSICAL;
        public boolean doHist = false;
        public VirialCO2H2OSC.Nonadditive nonAdditive = VirialCO2H2OSC.Nonadditive.NONE;
