...
public class VirialHSMixturePercolationOverlapSampling {

    public static void main(String[] args) throws IOException {

        VirialHSMixturePercolationOverlapSampling.VirialHSParam params = new VirialHSMixturePercolationOverlapSampling.VirialHSParam();
        if (args.length > 0) {
...
//        ((MCMoveStepTracker) sim.mcMoveTranslate[1].getTracker()).setAcceptanceTarget(targetAcceptance);
        ...
        ActivityIntegrate activityIntegrator = new ActivityIntegrate(sim.integrators[1]);
        if (false) {
...
//        sim.equilibrate(refFileName, (steps / subSteps) / 10);
        ...
        sim.setRefPref(1);
        activityIntegrator.setMaxSteps(steps / 10);
        activityIntegrator.actionPerformed();
        sim.accumulators[0].reset();
        sim.accumulators[1].reset();
        System.out.println("equilibration finished");

        sim.integratorOS.setNumSubSteps((int) blockSize);
        sim.setAccumulatorBlockSize(blockSize);
        activityIntegrator.setMaxSteps(steps);
