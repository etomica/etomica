 ...
 * March,9,2011
 */
public class VirialPh545 {


    public static void main(String[] args) {
        VirialPh3site545Param params = new VirialPh3site545Param();
        if (args.length > 0) {
        ...
        System.out.println("Phenanthrene(545) overlap sampling B"+nPoints+" at T="+temperature+" Kelvin");
        ...
        SpeciesFactory factoryAn = new SpeciesFactory() {
            public ISpecies makeSpecies(Space space) {
            	SpeciesPh3site545 species = new SpeciesPh3site545(space);
                      return species;
            }
        };
    ...
    // do simulation
        ...
        SpeciesPh3site545 speciesPh = (SpeciesPh3site545)sim.getSpecies(0);
        sim.integratorOS.setNumSubSteps(1000);

        IAtomType typeC = speciesPh.getCType();
        IAtomType typeCH = speciesPh.getCHType();
                
        pTarget.addPotential(p2C, ApiBuilder.makeIntergroupTypeIterator(new IAtomType[]{typeC, typeC}));
        pTarget.addPotential(pCCH, ApiBuilder.makeIntergroupTypeIterator(new IAtomType[]{typeC, typeCH}));
        pTarget.addPotential(p2CH, ApiBuilder.makeIntergroupTypeIterator(new IAtomType[]{typeCH, typeCH}));
        pTarget.addPotential(pCCH, ApiBuilder.makeIntergroupTypeIterator(new IAtomType[]{typeCH, typeC}));//switch
        // graphic part
        if (false) {
            double size = 10;
     ...
     */
    public static class VirialPh3site545Param extends ParameterBase {
