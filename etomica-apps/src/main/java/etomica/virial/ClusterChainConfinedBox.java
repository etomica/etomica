...
public class ClusterChainConfinedBox implements ClusterAbstract{
    protected final int n;
    protected double[][] pairSigma;
    protected final ClusterAbstract subCluster;

    public ClusterChainConfinedBox(int nPoints, double[][] pairSigma, ClusterAbstract subCluster ) {
        this.n = nPoints;
        this.pairSigma = pairSigma;
        this.subCluster = subCluster;
    }

    public ClusterChainConfinedBox(int nPoints, double[][] pairSigma) {
        this(nPoints, pairSigma, null);
    }

    @Override
    public ClusterAbstract makeCopy() {
        ClusterChainConfinedBox c = new ClusterChainConfinedBox(n, pairSigma, subCluster.makeCopy());
        c.setTemperature(1);
        return c;
    }
...
//        System.out.println("Resultant"+resultant);
        if (resultant.getX(0) < (box.getBoundary().getBoxSize().getX(0))/2)
            if (resultant.getX(1) < (box.getBoundary().getBoxSize().getX(1))/2)
                if (resultant.getX(2) < (box.getBoundary().getBoxSize().getX(2))/2)
                    if (subCluster != null) {
