 ...
 */
public class MCMoveClusterWiggleMultiFasterer extends MCMoveMoleculeFasterer {
    ...
    public MCMoveClusterWiggleMultiFasterer(IRandom random, PotentialCompute pc, Box box) {
        super(random, pc, box);
        setStepSize(1.0);
        this.pc = pc;
...
//                System.out.println("middle move "+j+" "+position);
            ...
            for (IAtom iAtom : childList) {
                iAtom.getPosition().ME(work1);
            }
            if (Debug.ON && Debug.DEBUG_NOW) {
                if (j > 0) {
