...
public class BigDecimalMath {
     ...
     * /library/books/sci/math/MiscellaneousMathematicalConstants/chap35.html
     */
    ...
    static public BigDecimal gamma(MathContext mc) {
        ...
        if (mc.getPrecision() < GAMMA.precision())
            ...
            /*
             * how many terms: zeta-1 falls as 1/2^(2n+1), so the terms drop
             * faster than 1/2^(4n+2). Set 1/2^(4kmax+2) < eps. Leading term
            ...
            for (int n = 1;; n++) {
                /*
                 * zeta is close to 1. Division of zeta-1 through 4^n*(2n+1)
                 * means divion through roughly 2^(2n+1)
                 */
    ...
    static public BigDecimal sqrt(final BigDecimal x, final MathContext mc) {
        if (x.compareTo(BigDecimal.ZERO) < 0)
            throw new ArithmeticException("negative argument " + x.toString()
                    + " of square root");
        if (x.abs().subtract(new BigDecimal(Math.pow(10., -mc.getPrecision())))
        ...
        MathContext locmc = new MathContext(mc.getPrecision() + 2, mc
                .getRoundingMode());
        ...
        for (;;) {
             ...
             */
            if (Math.abs(BigDecimal.ONE.subtract(
                    x.divide(s.pow(2, locmc), locmc)).doubleValue()) < eps)
                break;
    ...
    static public BigDecimal hypot(final BigDecimal x, final BigDecimal y) {
         ...
         */
        BigDecimal zerr = x.abs().multiply(x.ulp()).add(
                y.abs().multiply(y.ulp()));
         ...
         */
        mc = new MathContext(err2prec(z.doubleValue(), 0.5 * zerr.doubleValue()
                / z.doubleValue()));
        return z.round(mc);
    ...
    static public BigDecimal hypot(final int n, final BigDecimal x) {
         ...
         * zerr holds 2*x*xerr.
         */
        mc = new MathContext(err2prec(z.doubleValue(), 0.5 * zerr
                / z.doubleValue()));
        return z.round(mc);
    } /* BigDecimalMath.hypot */
    ...
    static public BigDecimal exp(BigDecimal x) {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
             ...
             * information.
             */
            return scalePrec(BigDecimal.ONE, -(int) (Math.log10(x.ulp()
                    .doubleValue())));
        } else {
            /*
            ...
            if (Math.pow(xDbl, TAYLOR_NTERM) < TAYLOR_NTERM
                    * (TAYLOR_NTERM - 1.0) * (TAYLOR_NTERM - 2.0) * xUlpDbl) {
                /*
                 ...
                 */
                MathContext mcTay = new MathContext(err2prec(1., xUlpDbl
                        / TAYLOR_NTERM));
                for (int i = 1; i <= TAYLOR_NTERM; i++) {
                    ifac = ifac.multiply(new BigInteger("" + i));
                    xpowi = xpowi.multiply(x);
                    final BigDecimal c = xpowi.divide(new BigDecimal(ifac),
                            mcTay);
                    resul = resul.add(c);
                    if (Math.abs(xpowi.doubleValue()) < i
                            && Math.abs(c.doubleValue()) < 0.5 * xUlpDbl)
                        break;
                }
                /*
                 ...
                 */
                int exSc = (int) (1.0 - Math.log10(TAYLOR_NTERM
                        * (TAYLOR_NTERM - 1.0) * (TAYLOR_NTERM - 2.0) * xUlpDbl
                        / Math.pow(xDbl, TAYLOR_NTERM))
                        / (TAYLOR_NTERM - 1.0));
                ...
                while (exSc > 0) {
                    ...
                    exSc -= exsub;
                    MathContext mctmp = new MathContext(expxby10.precision()
                            - exsub + 2);
                    int pex = 1;
                    while (exsub-- > 0)
                        pex *= 10;
    ...
    static public BigDecimal log(BigDecimal x) {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            BigDecimal zpown = z;
            double eps = 0.5 * x.ulp().doubleValue()
            / Math.abs(x.doubleValue());
            BigDecimal resul = z;
            for (int k = 2;; k++) {
             ...
             * xUlpDbl/xDbl .
             */
            MathContext mc = new MathContext(err2prec(resul.doubleValue(),
                    xUlpDbl / xDbl));
            return resul.round(mc);
        }
    } /* BigDecimalMath.log */
    ...
    static public BigDecimal log(int n, final MathContext mc) {
        ...
        if (n <= 0)
            ...
            if (mc.getPrecision() < LOG2.precision())
                ...
                /*
                 * Broadhurst <a
                 * href="http://arxiv.org/abs/math/9803067">arXiv:math
                 * /9803067</a> Error propagation: the error in log(2) is twice
                 * the error in S(2,-5,...).
                 */
                int[] a = { 2, -5, -2, -7, -2, -5, 2, -3 };
                BigDecimal S = broadhurstBBP(2, 1, a, new MathContext(1 + mc
                        .getPrecision()));
             ...
             * count of terms to estimate the precision (drop the favorable
             * additional 1/k here): 0.013^k <= 10^(-precision), so
             * k*log10(0.013) <= -precision so k>= precision/1.87.
             */
            int kmax = (int) (mc.getPrecision() / 1.87);
            MathContext mcloc = new MathContext(mc.getPrecision() + 1
                    + (int) (Math.log10(kmax * 0.693 / 1.098)));
            BigDecimal log3 = multiplyRound(log(2, mcloc), 19);

            /*
             * log3 is roughly 1, so absolute and relative error are the same.
             * The result will be divided by 12, so a conservative error is the
             * one already found in mc
             */
             ...
             * count of terms to estimate the precision (drop the favorable
             * additional 1/k here): 0.046^k <= 10^(-precision), so
             * k*log10(0.046) <= -precision so k>= precision/1.33.
             */
            int kmax = (int) (mc.getPrecision() / 1.33);
            MathContext mcloc = new MathContext(mc.getPrecision() + 1
                    + (int) (Math.log10(kmax * 0.693 / 1.609)));
            BigDecimal log5 = multiplyRound(log(2, mcloc), 14);

            /*
             * log5 is roughly 1.6, so absolute and relative error are the same.
             * The result will be divided by 6, so a conservative error is the
             * one already found in mc
             */
             ...
             * count of terms to estimate the precision (drop the favorable
             * additional 1/k here): 0.125^k <= 10^(-precision), so
             * k*log10(0.125) <= -precision so k>= precision/0.903.
             */
            int kmax = (int) (mc.getPrecision() / 0.903);
            MathContext mcloc = new MathContext(mc.getPrecision() + 1
                    + (int) (Math.log10(kmax * 3 * 0.693 / 1.098)));
             ...
             * signature (implemented) or decompose n into Ifactors and use an
             * implemenation of all the prime bases. Estimate of the result;
             * convert the mc argument to an absolute error eps log(n+errn) =
             * log(n)+errn/n = log(n)+eps
             */
             ...
             * error in n
             */
            final MathContext mcloc = new MathContext(1 + err2prec((double) n,
                    eps));
            /*
    ...
    static public BigDecimal log(final Rational r, final MathContext mc) {
        ...
        if (r.compareTo(Rational.ZERO) <= 0)
             ...
             * error in the result. eps contains the required absolute error of
             * the result, epsr/r.
             */
             ...
             * in r, given that epsr/r is also the relative precision of r. Add
             * one safety digit.
             */
    ...
    static public BigDecimal pow(final BigDecimal x, final BigDecimal y) {
        if (x.compareTo(BigDecimal.ZERO) < 0)
             ...
             * |log(x)*err(y)|+|y*err(x)/x|
             */
            double errR = Math.abs(logx.doubleValue() * y.ulp().doubleValue()
                    / 2.)
                    + Math.abs(y.doubleValue() * x.ulp().doubleValue() / 2.
                            / x.doubleValue());
    ...
    static public BigDecimal powRound(final BigDecimal x, final int n) {
        ...
        if (n == 1)
            ...
            MathContext mc = new MathContext(x.precision()
                    - (int) Math.log10((double) (Math.abs(n))));
            if (n > 0)
                return x.pow(n, mc);
            else
    ...
    static public BigDecimal powRound(final BigDecimal x, final BigInteger n) {
         ...
         */
        if (n.compareTo(Rational.MAX_INT) > 0
                || n.compareTo(Rational.MIN_INT) < 0)
            throw new ProviderException("Not implemented: big power "
                    + n.toString());
        else
     ...
     *            according to the parity of the exponent.
     * @param q
     *            The exponent.
     * @return x^q.
     * @since 2010-05-26
    ...
    static public BigDecimal powRound(final BigDecimal x, final Rational q) {
        ...
        if (q.compareTo(BigInteger.ONE) == 0)
             ...
             * normalize() has been called during constrution etc.
             */
         ...
         */
            ...
            if (q.isIntegerFrac()) {
                /*
                 * Newton method with first estimate in double precision. The
                 * disadvantage of this first line here is that the result must
                 * fit in the standard range of double precision numbers
                 * exponents.
                 */
                ...
                /*
                 * The error in x^q is q*x^(q-1)*Delta(x). The relative error is
                 * q*Delta(x)/x, q times the relative error of x.
                 */
                BigDecimal reserr = new BigDecimal(0.5
                        * q.abs().doubleValue()
                        * x.ulp().divide(x.abs(), MathContext.DECIMAL64)
                        .doubleValue());

                /*
                 * The main point in branching the cases above is that this
                 * conversion will succeed for numerator and denominator of q.
                 */
                int qa = q.a.intValue();
                int qb = q.b.intValue();
                ...
                for (;;) {
                     ...
                     * of the powers of the new estimate are done in full
                     * precision calling BigDecimal.pow(), which becomes slow if
                     * the denominator of q is large.
                     */
                     ...
                     * reserr, |eps/res| < reserr
                     */
                    if (eps.divide(res, MathContext.DECIMAL64).abs().compareTo(
                            reserr) < 0) {
                        /*
                         * delete the bits of extra precision kept in this
                         * working copy.
                         */
                        MathContext mc = new MathContext(err2prec(reserr
                                .doubleValue()));
                        return res.round(mc);
                    }
                }
                 ...
                 * q to a floating point number such that its relative error
                 * becomes negligible: Delta(q)/q << Delta(x)/x/log(x) .
                 */
                int precq = 3 + err2prec((x.ulp().divide(x,
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (res.compareTo(p) > 0) {
                ...
                if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
                     ...
                     */
                    int k = (int) (res.precision() / Math.log10(1.0 / res
                            .doubleValue())) / 2;
                    MathContext mcTay = new MathContext(err2prec(res
                            .doubleValue(), xUlpDbl / k));
                    for (int i = 1;; i++) {
                        /*
                        ...
                        BigDecimal corr = xpowi.divide(new BigDecimal(ifac),
                                mcTay);
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (res.compareTo(p) > 0) {
                ...
                if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
                     ...
                     */
                    double xUlpDbl = 0.5 * res.ulp().doubleValue()
                    * res.doubleValue();
                     ...
                     */
                    int k = (int) (Math.log(xUlpDbl) / Math.log(res
                            .doubleValue())) / 2;
                    MathContext mcTay = new MathContext(err2prec(1., xUlpDbl
                            / k));
                    for (int i = 1;; i++) {
        ...
        if (x.compareTo(BigDecimal.ZERO) == 0)
            ...
            if (xDbl > 0.8) {
                ...
                for (int i = 2;; i++) {
                    ...
                    f = f.multiply(fourn).multiply(
                            fourn.subtract(BigInteger.ONE)).divide(fac);
    ...
    static public BigDecimal cot(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) == 0) {
            ...
            MathContext mc = new MathContext(err2prec(xhighpr.doubleValue(),
                    eps));
    ...
    static public BigDecimal asin(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ONE) > 0
            ...
            final double xDbl = x.doubleValue();
            final double xUlpDbl = x.ulp().doubleValue() / 2.;
            final double eps = xUlpDbl / 2.
            / Math.sqrt(1. - Math.pow(xDbl, 2.));
            ...
            for (int i = 1;; i++) {
                ...
                BigDecimal c = divideRound(multiplyRound(xpowi, ifacN), ifacD
                        .multiply(new BigInteger("" + (2 * i + 1))));
                resul = resul.add(c);
                /*
    ...
    static public BigDecimal atan(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0) {
             ...
             * arctan(2x/(1-x^2)) = arctan(y). x=(sqrt(1+y^2)-1)/y This maps
             * 0<=y<=3 to 0<=x<=0.73 roughly. Temporarily with 2 protectionist
             * digits.
             */
            BigDecimal y = scalePrec(x, 2);
            BigDecimal newx = divideRound(hypot(1, y).subtract(BigDecimal.ONE),
                    y);
             ...
             * of the ulp.
             */
            double eps = x.ulp().doubleValue()
            / (2.0 * Math.hypot(1.0, x.doubleValue()));
            MathContext mc = new MathContext(err2prec(resul.doubleValue(), eps));
            return resul.round(mc);
    ...
    static public BigDecimal cosh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (x.doubleValue() > 1.5) {
                 ...
                 * xUlpDbl. We need at most k terms to push x^(2k)/(2k)! below
                 * this value. x^(2k) < xUlpDbl; (2k)*log(x) < log(xUlpDbl);
                 */
                int k = (int) (Math.log(xUlpDbl) / Math.log(x.doubleValue())) / 2;

                /*
                 * The individual terms are all smaller than 1, so an estimate
                 * of 1.0 for the absolute value will give a safe relative error
                 * estimate for the indivdual terms
                 */
    ...
    static public BigDecimal sinh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (x.doubleValue() > 2.4) {
                ...
                BigDecimal resul = sinh(xhalf).multiply(cosh(xhalf)).multiply(
                        two);
                /*
                 * The error in the result is set by the error in x itself. The
                 * first derivative of sinh(x) is cosh(x), so the absolute error
                 * in the result is cosh(x)*errx, and the relative error is
                 * coth(x)*errx = errx/tanh(x)
                 */
                double eps = Math.tanh(x.doubleValue());
                MathContext mc = new MathContext(err2prec(0.5
                        * x.ulp().doubleValue() / eps));
                return resul.round(mc);
            } else {
                BigDecimal xhighpr = scalePrec(x, 2);
                ...
                /*
                 * The error in the result is set by the error in x itself. We
                 * need at most k terms to squeeze x^(2k+1)/(2k+1)! below this
                 * value. x^(2k+1) < x.ulp; (2k+1)*log10(x) < -x.precision;
                 * 2k*log10(x)< -x.precision; 2k*(-log10(x)) > x.precision;
                 * 2k*log10(1/x) > x.precision
                 */
                int k = (int) (x.precision() / Math.log10(1.0 / xhighpr
                        .doubleValue())) / 2;
                MathContext mcTay = new MathContext(err2prec(x.doubleValue(),
                        xUlpDbl / k));
                for (int i = 1;; i++) {
                    /*
                     * TBD: at which precision will 2*i or 2*i+1 overflow?
    ...
    static public BigDecimal tanh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ZERO) < 0)
             ...
             */
            double eps = 0.5 * x.ulp().doubleValue()
            / Math.pow(Math.cosh(x.doubleValue()), 2.0);
            MathContext mc = new MathContext(err2prec(Math
                    .tanh(x.doubleValue()), eps));
            return BigDecimal.ONE.subtract(exp2x).divide(
                    BigDecimal.ONE.add(exp2x), mc);
        }
    } /* BigDecimalMath.tanh */
    ...
    static public BigDecimal acosh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ONE) < 0)
             ...
             */
            BigDecimal logx = log(sqrt(xhighpr.pow(2).subtract(BigDecimal.ONE))
                    .add(xhighpr));
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (x.compareTo(BigDecimal.ONE) != 0) {
                ...
                for (int n = 2;; n++) {
                     ...
                     * relative errors add. so the requirement in the relative
                     * error of zeta(n)-1 is that this is somewhat smaller than
                     * the relative error in z^n/n (the absolute error of
                     * thelatter is the absolute error in z)
                     */
                    BigDecimal c = divideRound(z.pow(n, mcloc), n);
                    MathContext m = new MathContext(err2prec(n
                            * z.ulp().doubleValue() / 2. / z.doubleValue()));
                    c = c.round(m);

                    /*
                     * At larger n, zeta(n)-1 is roughly 1/2^n. The product is
                     * c/2^n. The relative error in c is c.ulp/2/c . The error
                     * in the product should be small versus eps/10. Error from
                     * 1/2^n is c*err(sigma-1). We need a relative error of
                     * zeta-1 of the order of c.ulp/50/c. This is an absolute
                     * error in zeta-1 of c.ulp/50/c/2^n, and also the absolute
                     * error in zeta, because zeta is of the order of 1.
                     */
                    if (eps / 100. / c.doubleValue() < 0.01)
                        m = new MathContext(err2prec(eps / 100.
                                / c.doubleValue()));
                    else
                        m = new MathContext(2);
                    /* zeta(n) -1 */
    ...
    static public BigDecimal Gamma(final Rational q, final MathContext mc) {
        if (q.isBigInteger()) {
             ...
             * Delta(x) such that this is equivalent to mc: Delta(x) =
             * precision/psi(x).
             */
    ...
    static public BigDecimal pochhammer(final BigDecimal x, final int n) {
        ...
        if (n < 0)
            throw new ProviderException(
                    "Not implemented: pochhammer with negative index " + n);
        else if (n == 0)
            return BigDecimal.ONE;
        else {
            /*
    ...
    static public BigDecimal mod2pi(BigDecimal x) {
         ...
         * x.ulp()/2.
         */
        mc = new MathContext(err2prec(res.doubleValue(),
                x.ulp().doubleValue() / 2.));
        return res.round(mc);
    } /* mod2pi */
    ...
    static public BigDecimal modpi(BigDecimal x) {
         ...
         * and not compromised by the precision of pi, so the ulp of pi*k should
         * match the ulp of x. First get a guess of k to figure out how many
         * digits of pi are needed.
         */
    ...
    static public BigDecimal zeta(final int n, final MathContext mc) {
        if (n <= 0)
            throw new ProviderException(
                    "Not implemented: zeta at negative argument " + n);
        if (n == 1)
        ...
        if (n % 2 == 0) {
             ...
             * n times error in pi times pi^(n-1). Relative error is
             * n*error(pi)/pi, requested by mc. Need one more digit in pi if
             * n=10, two digits if n=100 etc, and add one extra digit.
             */
            MathContext mcpi = new MathContext(mc.getPrecision()
                    + (int) (Math.log10(10.0 * n)));
             ...
             * is roughly 0.031 9*2048*S51/6265 = -3.28. 7*2038*S53/61651= 5.07.
             * 738*2048*S55/61651= 0.747. The result is of the order 1.03, so we
             * add 2 digits to S51 and S52 and one digit to S55.
             */
            int[] a51 = { 31, -1614, -31, -6212, -31, -1614, 31, 74552 };
            int[] a53 = { 173, 284, -173, -457, -173, 284, 173, -111 };
            int[] a55 = { 1, 0, -1, -1, -1, 0, 1, 1 };
            BigDecimal S51 = broadhurstBBP(5, 1, a51, new MathContext(2 + mc
                    .getPrecision()));
            BigDecimal S53 = broadhurstBBP(5, 3, a53, new MathContext(2 + mc
                    .getPrecision()));
            BigDecimal S55 = broadhurstBBP(5, 5, a55, new MathContext(1 + mc
                    .getPrecision()));
            ...
            for (int npr = 0; npr <= (n + 1) / 2; npr++) {
                Rational b = bern.at(2 * npr)
                .multiply(bern.at(n + 1 - 2 * npr));
             ...
             * range 0.003 to 0.009. So the precision here is matching the
             * precisionn requested by mc, and the precision requested for 2*pi
             * is in absolute terms adjusted.
             */
            MathContext mcloc = new MathContext(2 + mc.getPrecision()
                    + (int) (Math.log10((double) (n))));
            ...
            if (n % 4 == 3) {
                /*
                 * since the argument n is at least 7 here, the drop of the
                 * terms is at rather constant pace at least 10^-3, for example
                 * 0.0018, 0.2e-7, 0.29e-11, 0.74e-15 etc for npr=1,2,3.... We
                 * want 2 times these terms fall below eps/10.
                 */
                int kmax = mc.getPrecision() / 3;
                eps /= kmax;
                /*
                 * need an error of eps for 2/(exp(2pi)-1) = 0.0037 The absolute
                 * error is 4*exp(2pi)*err(pi)/(exp(2pi)-1)^2=0.0075*err(pi)
                 */
                BigDecimal exp2p = pi(new MathContext(3 + err2prec(3.14,
                        eps / 0.0075)));
                ...
                /*
                 * since the argument n is at least 9 here, the drop of the
                 * terms is at rather constant pace at least 10^-3, for example
                 ...
                 * 2/(exp(2pi)-1)*(1+4*Pi/8/(1-exp(-2pi)) = 0.0096 at k=7 or =
                 * 0.00766 at k=13 for example. The absolute error is
                 * 0.017*err(pi) at k=9, 0.013*err(pi) at k=13, 0.012 at k=17
                 */
                BigDecimal twop = pi(new MathContext(3 + err2prec(3.14,
                        eps / 0.017)));
    ...
    static public double zeta1(final int n) {
         ...
         */
        final double[] zmin1 = { 0., 0., 6.449340668482264364724151666e-01,
        ...
        if (n < zmin1.length)
             ...
             * is computed, the equivalent accuracy in relative units is higher,
             * because zeta is around 1.
             */
    ...
    static public double psi(final double x) {
        ...
        if (x > 2.0) {
            ...
            final double[] psiT0 = { 9.67672245447621170427e-01,
                    -4.42763168983592106093e-01, 2.58499760955651010624e-01,
                    -1.63942705442406527504e-01, 1.07824050691262365757e-01,
                    -7.21995612564547109261e-02, 4.88042881641431072251e-02,
                    -3.31611264748473592923e-02, 2.25976482322181046596e-02,
                    -1.54247659049489591388e-02, 1.05387916166121753881e-02,
                    -7.20453438635686824097e-03, 4.92678139572985344635e-03,
                    -3.36980165543932808279e-03, 2.30512632673492783694e-03,
                    -1.57693677143019725927e-03, 1.07882520191629658069e-03,
                    -7.38070938996005129566e-04, 5.04953265834602035177e-04,
                    -3.45468025106307699556e-04, 2.36356015640270527924e-04,
                    -1.61706220919748034494e-04, 1.10633727687474109041e-04,
                    -7.56917958219506591924e-05, 5.17857579522208086899e-05,
                    -3.54300709476596063157e-05, 2.42400661186013176527e-05,
                    -1.65842422718541333752e-05, 1.13463845846638498067e-05,
                    -7.76281766846209442527e-06, 5.31106092088986338732e-06,
                    -3.63365078980104566837e-06, 2.48602273312953794890e-06,
                    -1.70085388543326065825e-06, 1.16366753635488427029e-06,
                    -7.96142543124197040035e-07, 5.44694193066944527850e-07,
                    -3.72661612834382295890e-07, 2.54962655202155425666e-07,
                    -1.74436951177277452181e-07, 1.19343948298302427790e-07,
                    -8.16511518948840884084e-08, 5.58629968353217144428e-08,
                    -3.82196006191749421243e-08, 2.61485769519618662795e-08,
                    -1.78899848649114926515e-08, 1.22397314032336619391e-08,
            ...
            /* 0.422... = 1 -gamma */
            return resul + 0.422784335098467139393487909917597568 + 0.5 / xmin1
            - 1. / (1 - xmin1 * xmin1) - Math.PI
     ...
     */
    static protected BigDecimal broadhurstBBP(final int n, final int p,
            final int a[], MathContext mc) {
        /*
        ...
        for (int c = 0;; c++) {
            ...
            if (Math.abs(r.doubleValue()) < eps)
                break;
            MathContext mcloc = new MathContext(1 + err2prec(r.doubleValue(),
                    eps));
     ...
     * @since 2009-07-30
     */
    static public BigDecimal subtractRound(final BigDecimal x,
            final BigDecimal y) {
     ...
     * @since 2010-07-19
     */
    static public BigComplex subtractRound(final BigComplex x,
            final BigComplex y) {
     ...
     * @since 2009-07-30
     */
    static public BigDecimal multiplyRound(final BigDecimal x,
            final BigDecimal y) {
     ...
     * @since 2010-07-19
     */
    static public BigComplex multiplyRound(final BigComplex x,
            final BigDecimal y) {
    ...
    static public BigComplex multiplyRound(final BigComplex x,
            final BigComplex y) {
        BigDecimal R = subtractRound(multiplyRound(x.re, y.re), multiplyRound(
                x.im, y.im));
        BigDecimal I = addRound(multiplyRound(x.re, y.im), multiplyRound(x.im,
                y.re));
        return new BigComplex(R, I);
    } /* multiplyRound */
    ...
    static public BigDecimal multiplyRound(final BigDecimal x,
         ...
         * The estimation of the absolute error in the result is |n*err(x)|
         */
        MathContext mc = new MathContext(n.compareTo(BigInteger.ZERO) != 0 ? x
                .precision() : 0);
        return resul.round(mc);
    }
    ...
    static public BigComplex invertRound(final BigComplex z) {
        if (z.im.compareTo(BigDecimal.ZERO) == 0) {
            ...
            return new BigComplex(BigDecimal.ZERO, BigDecimal.ONE.divide(z.im,
                    mc).negate());
        } else {
            /*
             * 1/(x.re+I*x.im) = 1/(x.re+x.im^2/x.re) - I /(x.im +x.re^2/x.im)
             */
            BigDecimal R = addRound(z.re, divideRound(
                    multiplyRound(z.im, z.im), z.re));
            BigDecimal I = addRound(z.im, divideRound(
                    multiplyRound(z.re, z.re), z.im));
    ...
    static public BigComplex divideRound(final BigInteger n, final BigComplex x) {
        ...
        if (x.im.compareTo(BigDecimal.ZERO) == 0)
            ...
            return new BigComplex(BigDecimal.ZERO, divideRound(n, x.im)
                    .negate());
