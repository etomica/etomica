 ...
 */
public class SimEinStep2Fasterer extends Simulation {
    ...
    public SimEinStep2Fasterer(Space _space, int numAtoms, double density, double temperature, double lambda, int exponent, double rc, boolean slanty) {
        super(_space);
...
//            primitive = new PrimitiveFcc(space, L*c);
        ...
        atomMove = new MCMoveAtomCoupledFasterer(lambda == 0 ? potentialMaster : potentialComputeComposite, random, space);
     ...
     * @see SimEinStep2Fasterer.SimOverlapParam
     */
...
//        lambda = c*(Math.pow(spring/c+1, f) - 1)
        ...
        final SimEinStep2Fasterer sim = new SimEinStep2Fasterer(Space.getInstance(3), numMolecules, density, temperature, lambda, exponentN, rc, slanty);
        ...
        final MeterPotentialEnergyFasterer meterPEHarmonic = new MeterPotentialEnergyFasterer(sim.potentialMasterHarmonic);
        int numBlocks = 100;
