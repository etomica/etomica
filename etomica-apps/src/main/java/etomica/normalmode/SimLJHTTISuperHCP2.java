...
public class SimLJHTTISuperHCP2 extends Simulation {
...
//        ((MCMoveStepTracker)atomMove.getTracker()).setNoisyAdjustment(true);
     ...
     * @see SimLJHTTISuperHCP2.SimOverlapParam
     */
    public static void main(String[] args) {
        ...
        if (args.length == 0) {
            ...
            params.rc = 3;
            params.rc0 = 3;
            params.bpharm = new double[]{9.550745288840814, 9.556020984242553, 9.559365557346363, 9.56183020258133, 9.562542814213337, 9.56286853476235, 9.562968096753176};
        } else {
        ...
        final SimLJHTTISuperHCP2 sim = new SimLJHTTISuperHCP2(Space.getInstance(3), numAtoms, density, alpha * Math.sqrt(8.0 / 3.0), temperature, rc * Math.pow(density, -1.0 / 3.0), ss, seeds);
        if (seeds == null) {
        ...
        meterP.setPotentialMaster(potentialMasterData);
        meterP.setTemperature(temperature);
        double pLat = meterP.getDataAsScalar();
        System.out.println("pLat " + pLat);
        MeterPressureTensor meterPT = new MeterPressureTensor(potentialMasterData, sim.space);
        meterPT.setBox(sim.box);
        meterPT.setTemperature(temperature);
        IData PT = meterPT.getData();
        double pzxy = PT.getValue(1);
        System.out.println(PT);
        System.out.println(pzxy);
        System.out.println(d.getValue(6 + 5));
        ...
        DataProcessorReweight puReweightPT = new DataProcessorReweight(temperature, energyFastCache, new double[]{uFacCut[1]}, sim.box, 1);
        DataPumpListener pumpPUPT = new DataPumpListener(meterPT, puReweightPT, interval);
        sim.integrator.getEventManager().addListener(pumpPUPT);
        final AccumulatorAverageCovariance avgPT = new AccumulatorAverageCovariance(blockSize);
        puReweightPT.setDataSink(avgPT);
        ...
        IData avgPTRawData = avgPT.getData(avgSolid.AVERAGE);
        IData errPTRawData = avgPT.getData(avgSolid.ERROR);
        IData covPTRawData = avgPT.getData(avgSolid.BLOCK_COVARIANCE);
        double avgWW = avgPTRawData.getValue(2);
        double avgPzxy1 = avgPTRawData.getValue(1);
        double errWW = errPTRawData.getValue(2);
        double errPzxy1 = errPTRawData.getValue(1);
        double corPzxyW = covPTRawData.getValue(5) / Math.sqrt(covPTRawData.getValue(4) * covPTRawData.getValue(8));
        if (errWW < 1e-7) corPzxyW = 0; // if this is sampled rc
        double errWWratio2 = errWW * errWW / (avgWW * avgWW);
//        double biasP = errWWratio2 - errPzxy1*errWW/(avgPzxy1*avgWW)*corPzxyW;
        errPzxy1 = Math.abs(avgPzxy1 / avgWW) * Math.sqrt(errPzxy1 * errPzxy1 / (avgPzxy1 * avgPzxy1) + errWWratio2 - 2 * errPzxy1 * errWW / (avgPzxy1 * avgWW) * corPzxyW);
        avgPzxy1 /= avgWW;
        System.out.println("PT 1 " + avgPzxy1 + " " + errPzxy1);
