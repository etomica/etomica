...
public class EwaldSummationLattice extends EwaldSummation {
    ...
    protected final AtomLeafAgentManager<Vector> latticeCoordinates;

    public EwaldSummationLattice(Box box,
                                 AtomLeafAgentManager<MyCharge> atomAgentManager, Space _space,
                                 double kCut, double rCutRealES, AtomLeafAgentManager<Vector> latticeCoordinates) {
    ...
    public double uReal() {
        ...
        for (int i = 0; i < nAtoms; i++) {//H
            ...
            Vector rA = latticeCoordinates.getAgent(atomA);
            ...
            for (int j = i; j < nAtoms; j++) {
                ...
                Vector rB = latticeCoordinates.getAgent(atomB);
                ldr.Ev1Mv2(rA, rB);
                shift.Ea1Tv1(-1, ldr);
