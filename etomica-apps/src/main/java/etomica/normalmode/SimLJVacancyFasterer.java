...
public class SimLJVacancyFasterer extends Simulation {

    public final PotentialComputePair potentialMasterPair;
    public final PotentialCompute potentialCompute;
    ...
    public SimLJVacancyFasterer(final int numAtoms, double temperature, double density, double rc, final int numV, final double bmu, final boolean ss, boolean shifted) {
        ...
        neighborManager = new NeighborCellManagerFasterer(getSpeciesManager(), box, 2, BondingInfo.noBonding());
        ...
        if (shifted) {
            ...
            PotentialCompute pcShift = new PotentialCompute() {
                @Override
                public void init() {
                ...
                public double computeAll(boolean doForces, PotentialCallback pc) {
                    ...
                    return uShift * numAtoms - 2 * (numAtoms - n) * uShift;
                }
            ...
            potentialCompute = new PotentialComputeAggregate(potentialMasterPair, pcShift);
        }
        else {
            potentialCompute = potentialMasterPair;
        }

        integrator = new IntegratorMCFasterer(potentialCompute, random, temperature, box);
        MCMoveAtomFasterer move = new MCMoveAtomFasterer(random, potentialCompute, box);
...
//        ((MCMoveStepTracker)move.getTracker()).setNoisyAdjustment(true);
        ...
        mcMoveID = new MCMoveInsertDeleteLatticeVacancyFasterer(potentialCompute, neighborManager, random, space, integrator, y, numAtoms, numV);
    ...
    public static void main(String[] args) {
        ...
        final SimLJVacancyFasterer sim = new SimLJVacancyFasterer(numAtoms, temperature, density, rc, numV, mu, ss, shifted);
        ...
        final MCMoveOverlapListenerFasterer mcMoveOverlapMeter = new MCMoveOverlapListenerFasterer(sim.mcMoveID, 11, daDef, numAtoms, 6);
        mcMoveOverlapMeter.setTemperature(temperature);
        sim.integrator.getMoveEventManager().addListener(mcMoveOverlapMeter);

        final MeterPressureFasterer meterP = new MeterPressureFasterer(sim.box, sim.potentialCompute);
        ...
        final MeterPotentialEnergyFromIntegratorFasterer meterU = new MeterPotentialEnergyFromIntegratorFasterer(sim.integrator);
        ...
        if (params.doReweight) {
            mcMoveBiasAction = new MCMoveIDBiasActionFasterer(sim.integrator, sim.mcMoveID, numV,
        ...
        if (graphics) {
            ...
            DataSourceMuRoot1Fasterer dsmr1 = new DataSourceMuRoot1Fasterer(mcMoveOverlapMeter, mu, pSplitter, Double.NaN, density, numAtoms/density);
            ...
            DataSourceMuRootFasterer.DataSourceMuRootDPressure dsmrDP = dsmr.new DataSourceMuRootDPressure();
            ...
            DataSourceMuRootFasterer.DataSourceMuRootDU dsmrDU = dsmr.new DataSourceMuRootDU();
