 ...
 */
public class DataSourceMSDAC implements IDataSource, ConfigurationStorage.ConfigurationStorageListener, DataSourceIndependent, Statefull {
    ...
    public IData getData() {
        ...
        double msd = msdSum / (numMsdSum*nMolecules);
        for (int i = 0; i < bacSum.length; i++) {
            long M = nMolecules*nSamples[i];
            y[i] = bacSum[i]/M - msd*msd;
//            System.out.println(bacSum[i]/M+" "+msd*msd+" "+y[i]);
            yErr[i] = Math.sqrt((bac2Sum[i]/M - y[i]*y[i]) / (M - 1));
        }
        return data;
    }
    ...
    public void newConfigruation() {
        ...
        for (int i = 0; i < configStorage.getLastConfigIndex(); i++) {
            ...
            if (step % (1L << x) == 0) {
                ...
                for (int j = 0; j < box.getMoleculeList().size(); j++) {
                    double dot = nowMSD[j] * computeMSD(iCoords, j, n);
                    bacSum[i] += dot;
                    bac2Sum[i] += dot*dot;
                }
                nSamples[i]++;
    ...
    protected double computeMSD(Vector[] rAtom, int jMol, int n) {
