 ...
 */
public class HSNPTFasterer extends Simulation {
    ...
    public HSNPTFasterer(Space _space, int numAtoms, double rho, boolean nvt, boolean fancyMove, double sigma2) {
        super(_space);
        ...
        potentialMaster = new PotentialMasterListFasterer(getSpeciesManager(), box, 1, neighborRangeFac * sigma, BondingInfo.noBonding());
        ...
        MCMoveAtomCoupledFasterer mcMove = new MCMoveAtomCoupledFasterer(potentialMaster, random, _space);
        ...
        if (!nvt) {
            ...
            if (fancyMove) {
                // fancy move
                mcMoveVolume = new MCMoveVolumeSolidFasterer(integrator, coordinateDefinition, getRandom(), space, p);
                ((MCMoveVolumeSolidFasterer) mcMoveVolume).setTemperature(1.0);
            } else {
                // standard move
    ...
    public static void main(String[] args) {
        ...
        final HSNPTFasterer sim = new HSNPTFasterer(params.numAtoms == 120 ? Space2D.getInstance() : Space3D.getInstance(), params.numAtoms, params.rho, params.nvt, params.fancyMove, params.sigma2);
...
//            sim.integrator.getEventManager().addListener(displacementMaxPump);
            ...
            MeterMaxExpansionFasterer meterMaxExpansion = new MeterMaxExpansionFasterer(sim.box, sim.potentialMaster.getNeighborManager().makeNeighborIterator());
