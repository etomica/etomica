...
package etomica.models.copy.molecularhma;
...
import etomica.models.clathrates.molecularhma.MeterDADBWaterTIP4P;
...
public class Clathrateenergyandcvcopy extends Simulation {
    ...
    public Clathrateenergyandcvcopy(Space space, double temperature, int numCells, double rCutRealES, double rCutLJ, boolean isIce, double kCut, double shakeTol, boolean unitCells, final boolean doTranslation, final boolean doRotation, double timeInterval) {
...
//////////minimized file
        ...
        latticeCoordinates = new MoleculeAgentManager(this, box, new MoleculeSiteSource(space, new MoleculePositionCOM(space), new Clathrateenergyandcvcopy.WaterOrientationDefinition(space)));
...
//        System.out.println("o at "+((IAtomPositioned)box.getLeafList().getAtom(2)).getPosition());
    ...
    public static void main(String[] args) {
        ...
        final Clathrateenergyandcvcopy sim = new Clathrateenergyandcvcopy(Space3D.getInstance(), temperature, numCells, rCutRealES, rCutLJ, isIce, kCut, shakeTol, uniteCells, doTranslation, doRotation, timeInterval);
        ...
        if (doMapping) {
            ...
            double mappingAverageanh = accumulatorAverageFixedDADB.getData(AccumulatorAverage.AVERAGE).getValue(1);
            double mappingErroranh = accumulatorAverageFixedDADB.getData(AccumulatorAverage.ERROR).getValue(1);
            double mappingCoranh = accumulatorAverageFixedDADB.getData(AccumulatorAverage.BLOCK_CORRELATION).getValue(1);

            double mappingAveragedelrphidelr = accumulatorAverageFixedDADB.getData(AccumulatorAverage.AVERAGE).getValue(2);
            double mappingErrordelrphidelr = accumulatorAverageFixedDADB.getData(AccumulatorAverage.ERROR).getValue(2);
            double mappingCordelrphidelr = accumulatorAverageFixedDADB.getData(AccumulatorAverage.BLOCK_CORRELATION).getValue(2);

            System.out.println("mappingAverage=\t" + mappingAverage +
                    "   mappingError=\t" + mappingError + " mappingCor=\t" + mappingCor + " time= " + totalTime+" "+mappingAverageanh+" "+mappingErroranh+" "+mappingAveragedelrphidelr+" "+mappingErrordelrphidelr);
        }
    ...
    public static class SimParams extends ParameterBase {
        ...
        public double timeInterval = 0.0022;
