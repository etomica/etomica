...
public class MeterMappedRdfnew implements IEtomicaDataSource, DataSourceIndependent, Serializable, AtomLeafAgentManager.AgentSource<IntegratorVelocityVerlet.MyAgent> {
    ...
    public MeterMappedRdfnew(double rcforHandfinmap, Space space, PotentialMaster potentialMaster, Box box, int nbins, double density) {
        this.space = space;
        this.box = box;
...
//            double vShell = space.sphereVolume(r[i]+dx2)-space.sphereVolume(r[i]-dx2);
           ...
           // y[i] =   (newgterm[i]) ;
            ...
            y[i] =  numAtoms * (numAtoms - 1)/(vol*vol) +(newgterm[i]) ;
            //    y[i] =  (gSum[i] / (norm)) ;
