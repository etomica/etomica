 ...
 * using mapped averaging
 * Does not exploit symmetry about centerline between walls
 */
public class MeterDensityProfileMA implements IDataSource, DataSourceIndependent {
    ...
    public MeterDensityProfileMA(PotentialCompute potentialCompute, Box box, double T, int d, int nBinsPerp) {
    ...
    public IData getData() {
        ...
        data.E(nMolecules/box.getBoundary().volume());
...
//            zfzsum += zi*fz;
            ...
            for(int iz=0; iz<nBinsPerp; iz++) {
                ...
                double zdot = -zi / Lz;
                if (zi > z) zdot += 1;
                zdot /= T * S;//S is area perp to z
                y[iz] += -zdot * fz;
            }
        }
//        if(Math.random() < 0.01) System.out.println(fzsum+", "+zfzsum/Lz/nMolecules/T/S);
        return data;
    }
