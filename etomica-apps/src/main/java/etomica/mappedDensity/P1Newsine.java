...
public class P1Newsine implements IPotentialAtomic, PotentialSoft {
    ...
    public P1Newsine(Space space, int n, double temperature) {
        this.n = n;
    ...
    public Vector[] gradient(IAtomList atoms) {
        ...
        gradient[0].setX(2, -temperature / (1.2 + Math.sin(arg * z)) * Math.cos(arg * z) * arg);
//        System.out.println(z+" "+energy(atoms)+" "+gradient[0].getX(2));
    ...
    public double energy(IAtomList atoms) {
        double z = atoms.get(0).getPosition().getX(2);
        return -temperature * Math.log(1.2 + Math.sin(2 * Math.PI * n * z / L));
    }
