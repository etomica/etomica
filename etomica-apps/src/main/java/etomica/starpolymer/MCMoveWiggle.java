...
public class MCMoveWiggle extends MCMoveMolecule {
    ...
    public MCMoveWiggle(IRandom random, PotentialMaster potentialMaster, int armLength, Space _space) {
        this(potentialMaster, random, 1.0, armLength, _space);
    }

    public MCMoveWiggle(PotentialMaster potentialMaster,
    ...
    public boolean doTrial() {
        ...
        while (true) {
            ...
            if (j > 0 && (j - 1) % armLength > 1) break;
        }
        selectedAtoms = childList.get(j);
//            System.out.println(selectedAtoms[i]+" "+j+" before "+selectedAtoms[i].coord.position());
        Vector position = selectedAtoms.getPosition();
...
//                System.out.println("middle move "+j+" "+position);
    ...
    public void rejectNotify() {
        ...
        selectedAtoms.getPosition().ME(translationVectors);
    }
