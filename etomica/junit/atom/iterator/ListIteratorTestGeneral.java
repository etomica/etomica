...
/**
 * Unit test for AtomIteratorList. Test that effect of direction, first, and
 * terminator are correct, and checks for appropriate behavior with or without
 * tabs in list.
 * 
 * @author Ken Benjamin
 *  
...
public class ListIteratorTestGeneral extends ListIteratorTest {
    ...
    public void iteratorStateTests(AtomList list) {
        ...
        if (UnitTest.VERBOSE)
            System.out.println("the current iterator direction is: "
                    + iterator.getDirection());
        assertEquals(iterator.getDirection(), IteratorDirective.UP);
        // set the first element in the iterator to be the first atom in the
        // list
        ...
        //		printLists(lister);
        // set the first element in the iterator to be the middle atom in the
        // list
         ...
         * System.out.println("iterator.setFirst equals last");
         * generalIteratorMethodTests(iterator); iteratorUpFirstNoArg=iterator;
         *///		printLists(lister);
        //Setting new list should keep direction same, but make header the
        // first and last
        ...
        /**
         * Sets the iteration direction to DOWN and test the methods in turn
         * with the first element equal to first, middle, and last.
         */
        ...
        if (UnitTest.VERBOSE)
            System.out
                    .println("Just tested with reverse collections and compared listUpFirst and listDownLast");
        //		printLists(lister);
        //		clearLists(lister);
        //		Commented out all calls to printLists and clearLists due to error
        // with lister, 12/06/04
        iterator.setDirection(IteratorDirective.UP);
