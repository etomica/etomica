 ...
 *
 */
public class B9Optimizer {
	
	public B9Optimizer(String filenameP, String filenameB){
		...
		pVirial = new double[pSimData.length];
	...
	public void calcpVirial(double bGuess){
		...
		for (int irho=0; irho< rho.length; irho++){
			...
			for (int i=0; i<bVirial.length;i++){
				pCalc += bVirial[i]*Math.pow(rho[irho],i);
			}
	...
	public double minimumScreening(double min, double max){
		...
		for (int i=0; i<nPoint; i++){
			...
			double TSS = 0.0;
			for (int j=0; j<pVirial.length; j++){
				diff = (pVirial[j]-pSimData[j][1]);
				TSS += diff*diff;
			}
			if(TSS < globalMin){
				globalMin = TSS;
	...
	public void optimizeHigherbVirial(double minb, double maxb){
		...
		while (true){
			...
			double TSS = 0.0;
			for (int i=0; i<pVirial.length; i++){
				diff = (pVirial[i]-pSimData[i][1]);
				TSS += diff*diff;
		...
		System.out.println(allTSS[0]+" "+allTSS[1]+" "+allTSS[2]+" "+bGuess);
	}
	...
	public static void main(String[] args){
		...
		B9Optimizer vOpt = new B9Optimizer(filenameP, filenameB);
			
		double x = vOpt.minimumScreening(-5, 5);
		double min = (1-0.05)*x;
		double max = (1+0.05)*x;
		
		vOpt.optimizeHigherbVirial(min, max);		
	
	}
	
	protected double diff;
	protected double[] bVirial, rho, pVirial;
	protected double[][] pSimData, bVirialFromFile;
	protected double bGuess;
	
	public static class VirialParam extends ParameterBase {
	        public String filenameP = "/tmp/data2048_n12";
	        public String filenameB = "/tmp/Bn12"; 
	}
}
