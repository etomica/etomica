...
public class CombiningRuleLorenzBerthelot extends CombiningRule {
    ...
    public double sigma(Atom[] pair) {
        double sigma1 = ((Agent)pair[0].agents[0]).sigma;
        double sigma2 = ((Agent)pair[0].agents[0]).sigma;
        return (sigma1 == sigma2) ? sigma1 : 0.5*(sigma1+sigma2);
    }
    
    public double epsilon(Atom[] pair) {
        double epsilon1 = ((Agent)pair[0].agents[0]).epsilon;
        double epsilon2 = ((Agent)pair[1].agents[0]).epsilon;
