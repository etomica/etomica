...
public class IntegerBravaisLattice implements AbstractLattice {
    ...
    private SiteList siteList;//used only by top-level lattice
    ...
    /**
     * Constructor for top-level lattice, with site coordinates generated by simple 
     * default IndexedCoordinate.Factory object.
     */
 ...
 //       setupNeighbors(rows);
    ...
    /**
     * Constructor for top-level lattice, with site coordinates generated by 
     * given IndexedCoordinate.Factory object.
     */
  ...
  //      setupNeighbors(rows);
    ...
    private IntegerBravaisLattice(IntegerBravaisLattice parent, 
        ...
        iterator = new SingletIterator(siteFactory.makeSite(top, coordFactory.makeCoordinate(coordIndex)));
        siteCount = 1;
        nRows = 0;
    }
    /**
    ...
    // sets iterator for top lattice as a list iterator, and sets up neighbors
    // for all sites using adjacency definition
    private void setup() {
        siteList = new SiteList();
        iterator.reset();
        while(iterator.hasNext()) {siteList.add(iterator.next());}
        iterator = new SiteIteratorList(siteList);
        SiteIteratorList iterator0 = new SiteIteratorList(siteList);
        iterator.reset();
        //define neighbor criterion as first-neighbor adjacency in lattice
        NeighborManager.Criterion criterion = new NeighborManager.Criterion() {
            public boolean areNeighbors(Site s1, Site s2) {
                return ((IndexedCoordinate)s1.coordinate()).isAdjacentTo((IndexedCoordinate)s2.coordinate());
            }
        };
        while(iterator.hasNext()) {
            Site site = iterator.next();
            site.neighborManager().setupNeighbors(iterator0, criterion);
        }
    }
...
/*    private void setupNeighbors(IntegerBravaisLattice[] rows) {
   ...
   //need to modify to handle pbcs
    ...
    private static final class SingletIterator implements SiteIterator {
        ...
        private final SiteLinker link;
        private boolean hasNext;
        public SingletIterator(Site s) {
            site = s; 
            hasNext = true; 
            link = new SiteLinker(s, null, null);
        }
        public boolean hasNext() {return hasNext;}
