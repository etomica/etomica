    ...
    public SimulationEditorPane(SimulationEditor ed, String t){
		...
		rightPaneList.addListSelectionListener(new MyListSelectionListener() {
		    public void valueChanged(javax.swing.event.ListSelectionEvent lse){
                ...
                propertySheet.setTarget((SimulationElement)obj);
                try {
...
//		if (getTitle() == "Species") makeDefineMoleculeButton();
                    ...
                    if ((getTitle() == "Phase") || (currentButton.cls != null)) {
	                    try {   // Try to make an instance of the selected class
	                        if (getTitle() == "Phase") {
	                            ...
	                            setComponent(phase);
	                            ((SimulationElement)getComponent()).setName(getTitle() + Integer.toString(IDnumber++));
	                        }
	                        else {
	                            setComponent(((Class)currentButton.cls).newInstance());
	                            ((SimulationElement)getComponent()).setName(((Class)currentButton.cls).getName().substring(8) + Integer.toString(IDnumber++));
    	                    }
    	                    Simulation currentSimulation = simulationEditor.getSimulation();
    	                    SimulationElement element = (SimulationElement)getComponent();
	    ...
	    remove.addActionListener(new MyActionListener(){
	        public void actionPerformed(ActionEvent e){
    	        Simulation currentSimulation = simulationEditor.getSimulation();
    	        SimulationElement element = (SimulationElement)componentList.getElementAt(getCurrentSelection());
