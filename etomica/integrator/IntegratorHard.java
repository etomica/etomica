...
public class IntegratorHard extends IntegratorMD implements AgentSource, AtomTypeAgentManager.AgentSource {
    ...
    public void doStepInternal() {
        ...
        while(collisionTimeStep < timeStep) {//advance to collision if occurs before remaining interval
            ...
            if (atoms.getAtomCount() == 1) {
                updateAtom((IAtomLeaf)atoms.getAtom(0));
            }
            else {
	...
	*/
    ...
    protected void updateAtoms(AtomPair colliders) {
        ...
        Agent agent = ((Agent)agentManager.getAgent((IAtomLeaf)colliders.atom0));
        ...
        agent = ((Agent)agentManager.getAgent((IAtomLeaf)colliders.atom1));
        if (agent.collisionPotential != null) {
            agent.eventLinker.remove();
        }
        agent.resetCollisionFull();
    ...
    protected void processReverseList() {
        ...
        for (int i=0; i<size; i++) {
            ...
            Agent agent = (Agent)agentManager.getAgent((IAtomLeaf)reverseAtom);
            if (agent.collisionPotential != null) {
                agent.eventLinker.remove();
            }
    ...
    public void resetCollisionTimes() {
        ...
        for (int iLeaf=0; iLeaf<nLeaf; iLeaf++) {
            ...
            ((Agent)agentManager.getAgent((IAtomLeaf)atom)).resetCollisionFull();
        }
    ...
    protected static final class CollisionHandlerUp implements PotentialCalculation, java.io.Serializable {
        ...
        public void setAtom(IAtom a) {
            atom1 = a;
            aia = (Agent)integratorAgentManager.getAgent((IAtomLeaf)a);
	...
	private static final class CollisionHandlerDown implements PotentialCalculation, java.io.Serializable {
		...
		public void doCalculation(IAtomSet atoms, IPotential potential) {
			...
			if(collisionTime < Double.POSITIVE_INFINITY) {
				Agent aia = (Agent)integratorAgentManager.getAgent((IAtomLeaf)atoms.getAtom(0));
