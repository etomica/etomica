...
public class Plane {
     ...
     * in-plane vector.
     */
    public Vector3D[] inPlaneVectors(Vector3D[] p) {
     ...
     */
    public Vector3D nearestPoint(Vector3D x0, Vector3D point) {
    ...
    public double dihedralAngle(Plane p) {
        Vector3D n = p.getNormalVector(null);
     ...
     */
    public Vector3D[] inPlaneSquare(Vector3D x0, double d, Vector3D[] s) {
        if(s == null || s.length != 4) s = new Vector3D[] {new Vector3D(), new Vector3D(), new Vector3D(), new Vector3D()};
    ...
    //    Plane plane = new Plane(5.0, 0.0, 0.0, 293.8);
        Vector3D[] p = plane.inPlaneVectors(null);
        Vector3D n = plane.getNormalVector(null);
