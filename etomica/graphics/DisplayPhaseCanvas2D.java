...
public class DisplayPhaseCanvas2D extends DisplayCanvas {
    ...
    protected void drawAtom(Graphics g, int origin[], AtomLeaf a) {
        ...
        if(a.getType() instanceof AtomTypeSphere) {
            ...
            if(drawOrientation) {
                double theta = ((ICoordinateAngular)a.getCoord()).orientation().angle()[0];
...
//        Vector vec2 = displayPhase.getPhase().getBoundary().centralImage(vec);
        ...
        if(displayPhase.getDrawOverflow()) {
            ...
            while(atomIterator.hasNext()) {
                ...
                float[][] shifts = boundary.getOverflowShifts(a.getCoord().position(),0.5*((AtomTypeSphere)a.getType()).getDiameter());  //should instead of radius have a size for all AtomC types
