...
public class DisplayBox extends Display {
    ...
    public void setBox(IBox p) {
        ...
        int boxX = (int)(box.getBoundary().getDimensions().x(0) * toPixels + 1);
        int boxY = 1;

        switch(space.D()) {
            case 3:
                boxY = (int)(box.getBoundary().getDimensions().x(1) * toPixels);
                if (boxX > boxY) {
                    boxY = boxX;
                }
                else {
                ...
                break;
            case 2:
                boxY = (int)(box.getBoundary().getDimensions().x(1) * toPixels + 1);
...
//                    ((SimulationGraphic)parentSimulation()).panel().repaint();
    ...
    public void setBoxCanvas(DisplayCanvas boxCanvas) {
        ...
        int boxX = (int)(box.getBoundary().getDimensions().x(0) * pixel.toPixels() * scale + 1);
        int boxY = 1;

        switch(space.D()) {
            case 3:
                boxY = (int)(box.getBoundary().getDimensions().x(1) * pixel.toPixels() * scale + 1);
                if (boxX > boxY) {
                    boxY = boxX;
                }
                else {
...
//                    ((SimulationGraphic)parentSimulation()).panel().repaint();
    ...
    */
    ...
    public void computeImageParameters2(int w, int h) {
        ...
        drawSize[0] = (int)(toPixels*getBox().getBoundary().getDimensions().x(0));
        drawSize[1] = (space.D()==1) ? drawingHeight: (int)(toPixels*getBox().getBoundary().getDimensions().x(1));
    ...
    public void setPixelUnit(Pixel pixel) {
        ...
        int boxX = (int)(box.getBoundary().getDimensions().x(0) * pixel.toPixels());
        int boxY = 1;

        switch(space.D()) {
            case 3:
                boxY = (int)(box.getBoundary().getDimensions().x(1) * pixel.toPixels());
                if (boxX > boxY) {
                    boxY = boxX;
                }
                else {
