...
public class SpeciesChain extends Species {
  ...
  public final double getAtomsPerMolecule() {return nAtomsPerMolecule;}
  public final void setAtomsPerMolecule(int n) {
    nAtomsPerMolecule = n;
    initializeMolecules();
  }
...
//   setSigma(sig);
    ...
    outer: for ( ix=0; ix<moleculeColumns; ix++) {
      inner: for( iy=0; iy<moleculeRows; iy++) {
        Atom a0 = molecule[i].atom[0];
	    a0.r[0] = ix*moleculeInitialSpacingX + moleculeColumnsShift + speciesOrigin[0] + 0.5*e[0]*L;
	    a0.r[1] = iy*moleculeInitialSpacingY + moleculeRowsShift + speciesOrigin[1] + 0.5*e[1]*L;
        for(int k=1; k<nAtomsPerMolecule; k++) {
            Atom a = molecule[i].atom[k];
   	        a.r[0] = a0.r[0] - e[0]*L;
	        a.r[1] = a0.r[1] - e[1]*L;
