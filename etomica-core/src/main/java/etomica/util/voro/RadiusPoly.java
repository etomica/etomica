...
public class RadiusPoly implements Radius {
     ...
     * plane bounds check. */
    public void r_prime(double rv) {r_val=1+r_mul/rv;}
    ...
    public boolean r_ctest(double crs,double mrs) {return crs+r_mul>Math.sqrt(mrs*crs);}
    /** Scales a plane displacement during a plane bounds check.
     * \param[in] lrs the plane displacement.
     * \return The scaled value. */
    public double r_cutoff(double lrs) {return lrs*r_val;}
    ...
    public double r_max_add(double rs) {return rs+max_radius*max_radius;}
    ...
    public boolean r_scale_check(double[] rs,double mrs,int ijk,int q) {
        double trs=rs[0];
        rs[0]+=r_rad-ppr[ijk][4*q+3]*ppr[ijk][4*q+3];
        return rs[0]<Math.sqrt(mrs*trs);
    }
    private double r_rad,r_mul,r_val;
