...
public class VoroBase {
    ...
    public boolean contains_neighbor(String format) {
        for (int i=0; i<format.length()-1; i++) {
            if (format.charAt(i)=='%') {
                i++;
                if (format.charAt(i) == 'n') return true;
            }
        }
        return false;
    }
    ...
    public VoroBase(int nx_,int ny_,int nz_,double boxx_,double boxy_,double boxz_) {
        nx = nx_;
        ny = ny_;
        nz = nz_;
        nxy = nx_*ny_;
        ...
        ysp = 1/boxy_;
        zsp = 1/boxz_;
        mrad = new double[WorkList.wl_hgridcu*WorkList.wl_seq_length];
    	final int b1=1<<21,b2=1<<22,b3=1<<24,b4=1<<25,b5=1<<27,b6=1<<28;
	    final double xstep=boxx/WorkList.wl_fgrid,ystep=boxy/WorkList.wl_fgrid,zstep=boxz/WorkList.wl_fgrid;
        int i,j,k,lx,ly,lz,q;
        int f;
        int e = 0; // wl
        double xlo,ylo,zlo,xhi,yhi,zhi,minr;
        int radp=0;  // mrad
        for(zlo=0,zhi=zstep,lz=0;lz<WorkList.wl_hgrid;zlo=zhi,zhi+=zstep,lz++) {
            for(ylo=0,yhi=ystep,ly=0;ly<WorkList.wl_hgrid;ylo=yhi,yhi+=ystep,ly++) {
                for(xlo=0,xhi=xstep,lx=0;lx<WorkList.wl_hgrid;xlo=xhi,xhi+=xstep,lx++) {
                    minr=Config.large_number;
                    for(q=wl[e]+1;q<WorkList.wl_seq_length;q++) {
                        f=wl[e+q];
                        i=(f&127)-64;
                        j=(f>>7&127)-64;
                        k=(f>>14&127)-64;
                        if((f&b2)==b2) {
                            minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i-1,j,k);
                            if((f&b1)==0) minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i+1,j,k);
                        } else if((f&b1)==b1) minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i+1,j,k);
                        if((f&b4)==b4) {
                            minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i,j-1,k);
                            if((f&b3)==0) minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i,j+1,k);
                        } else if((f&b3)==b3) minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i,j+1,k);
                        if((f&b6)==b6) {
                            minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i,j,k-1);
                            if((f&b5)==0) minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i,j,k+1);
                        } else if((f&b5)==b5) minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i,j,k+1);
                    }
                    q--;
                    while(q>0) {
                        mrad[radp+q] = minr;
                        ...
                        j=(f>>7&127)-64;
                        k=(f>>14&127)-64;
                        minr = compute_minimum(minr,xlo,xhi,ylo,yhi,zlo,zhi,i,j,k);
                        q--;
                    }
                    mrad[radp] = minr;
                    e+=WorkList.wl_seq_length;
                    radp+=WorkList.wl_seq_length;
                }
            }
        }
    }
    ...
    protected int step_int(double a) {return a<0?((int)a)-1:(int)a;}
     ...
     * numbers. */
    protected int step_mod(int a,int b) {return a>=0?a%b:b-1-(b-1-a)%b;}
     ...
     * numbers. */
    protected int step_div(int a,int b) {return a>=0?a/b:-1+(a+1)/b;}
    ...
    private double compute_minimum(double minr, double xlo,double xhi,double ylo,double yhi,double zlo,double zhi,int ti,int tj,int tk) {
        double radsq,temp;
    ...
    public static final int[] wl= new int[]{
