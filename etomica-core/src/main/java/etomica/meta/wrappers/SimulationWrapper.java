...
public class SimulationWrapper extends ObjectWrapper<Simulation> {
    ...
    protected double allCoords[][][] = new double[0][][];
    ...
    public double[][][] getAllCoordinates() {
        ...
        if(allCoords.length != sim.getBoxCount()) {
            allCoords = new double[sim.getBoxCount()][0][];
            for(int i=0; i< sim.getBoxCount(); i++) {
                IAtomList leafList = sim.getBox(i).getLeafList();
                if(allCoords[i].length != leafList.getAtomCount()) {
                    allCoords[i] = new double[leafList.getAtomCount()][sim.getSpace().D()];
                }
                for(int j=0; j<leafList.getAtomCount(); j++) {
                    leafList.getAtom(j).getPosition().assignTo(allCoords[i][j]);
                }
            }
        }
        return allCoords;
