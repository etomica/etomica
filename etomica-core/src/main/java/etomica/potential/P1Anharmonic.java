...
public class P1Anharmonic implements IPotential1 {
    ...
    public double udu(IAtom atom, Vector f) {
        ...
        f.PEa1Tv1(-1.0*(k2 + k4/6.0*dr.squared()), dr);
        return u;
    }
...
//        System.out.println(atom + "  " + atom.getPosition());
        ...
        double dr4 = dr2*dr2;
        double du = 1.0*(k2*dr2 + 1.0/6.0*k4*dr4);  //rdu: r (k2 r + 1/6 k4 r^3)
        double d2u = 1.0*(k2*dr2 + 1.0/2.0*k4*dr4); //r2d2u: r^2 (k2 + 1/2 k4 r^2)
        Hii.Ev1v2(dr, dr);
        Hii.TE((d2u-du)/dr4);
