...
public class PDBBuilder {
    ...
    protected static void parseLine(String line, SpeciesBuilder speciesBuilder, Map<String, AtomType > typeMap, Map<Integer, String> atomMap , Map<Integer, Vector> positions, Map<Integer, Vector> positionsNew) {
        ...
        if (line.substring(0, 6).equals("HETATM")) {
            ...
            // Add the atomic weight to the molecular weight
    ...
    public static ISpecies getSpeciesNew (String confName){
        ...
        ArrayList<ArrayList<Integer>> connectedAtoms = PDBBuilder.getConnectivity();
        ...
        System.out.println("To print aromatic ");
        //aromaticCarbIdentifier();
        //uniqueAtomListUpdater(connectivity, atomMap);
        //System.out.println(positions);
        //System.out.println(positionsNew);
    ...
    public static ArrayList<Integer> getBonding (ArrayList<ArrayList<Integer>> connectivity, Map<Integer, String> atomMap){
        ...
        bondList = singleDoubleBondIdentifier(connectivity, atomMap );
        return bondList;
    }
    ...
    public static Set<String> uniqueElementAtomTypeConverter(ArrayList<ArrayList<Integer>> connectivity, Map<Integer, String> atomMap){
        Set<String> uniqueAtoms = uniqueElementIdentifier();
        for(String atom : uniqueAtoms){
            System.out.println(atom + " atoms");
            String newAtom = atom.substring(9, atom.length() - 1);
            AtomType atomType = AtomType.simple(newAtom);
            //System.out.println(atom + " atoms" + " " + newAtom + " NewAtoms" + " " + atomType + " atomType");
        }
        //int i = uniqueAtomListUpdater(connectivity, atomMap);
        /*if(i >0){
            AtomType atomName = AtomType.simple("C_Ar");
            uniqueAtoms.add(String.valueOf(atomName));
            System.out.println("Aromatics present");
            System.out.println(uniqueAtoms + " uniqueElementAtomTypeConverter");
        }else {
            System.out.println(uniqueAtoms + " uniqueElementAtomTypeConverter");
        }*/
    ...
    public static ArrayList<Integer> aromaticCarbIdentifier(){
        ArrayList<Integer>carbonAromatic = new ArrayList<>();
        ArrayList<Integer> bondList = singleDoubleBondIdentifier(connectivity, atomMap);
        System.out.println(bondList + " singleDoubleBondIdentifier");
        //System.out.println(bondList + "bondlist");;
        //System.out.println(atomMap);
        for (int i =0; i < connectivityModified.size(); i++){
            int neighbourDoubleBondCounter = 0;
            for(int j =0; j< connectivityModified.get(i).size(); j++){
                if (bondList.get(connectivityModified.get(i).get(j)) ==2){
                    neighbourDoubleBondCounter++;
                }
                //System.out.println("j "+j + " " +connectivityModified.get(i).get(j) +" "+ bondList.get(connectivityModified.get(i).get(j)));
            }
            System.out.println(connectivityModified.get(i) +" "+ bondList.get(i) + " Neighour double bond " + neighbourDoubleBondCounter +" "+ atomMapModified.get(i));
        }
        for(int i= 0; i< connectivityModified.size(); i++){
            //System.out.println(bondList.get(i) + " i before loop");
            //System.out.println(connectivityModified.get(i));
            int numBonds = 0;
            /*for(int j =0; j< connectivityModified.get(i).size(); j++){
                //System.out.println(bondList);
                int numberInArray = connectivityModified.get(i).get(j);
                int valueInBondList = bondList.get(numberInArray);
                //System.out.println(numberInArray + " " + valueInBondList);
                if(valueInBondList == 2){
                    numBonds++;
                }
            }
            System.out.println(numBonds + " numBonds");
            String atomName = atomMapModified.get(i);
            System.out.println(atomName + " " + i);
            if(numBonds >2){
                System.out.println(i+" i in atommore than 2 bonds ");
                carbonAromatic.add(i, 1);
                System.out.println(" numBonds greater than 2 for: " + atomMapModified.get(i) + " " + (i));
            } else if (!atomName.equals("C")) {
                System.out.println(i + " atoms not Carbon");
                carbonAromatic.add(i, -1);
            } else {
                System.out.println(i + " not aromatic");
                carbonAromatic.add(i, 0);
            }
            System.out.println("New Element");*/
        }
        System.out.println(atomMapModified);
        System.out.println(carbonAromatic + " carbonAromatic");
        return carbonAromatic;
    }
    /* public static int uniqueAtomListUpdater(){
         ArrayList<Integer>carbonAromatic = aromaticCarbIdentifier();
         Set<String> uniqueAtoms = uniqueElementIdentifier();
         //System.out.println(uniqueAtoms + "uniqueAtoms");
         int numCarbAromatic = 0;
         for(int i =0; i<carbonAromatic.size(); i++){
             if(carbonAromatic.get(i) ==1){
                 numCarbAromatic++;
             }
         }
         Set<AtomType> carbonAromaticAtomList = new HashSet<>();
        for(int i=0; i< atomIdentifierMap.size(); i++){
            //System.out.println(carbonAromatic.get(i)+" "+ atomIdentifierMap.get(i));
            if( carbonAromatic.get(i) == 1){
                carbonAromaticAtomList.add(AtomType.simple("C_Ar"));
            } else {
                carbonAromaticAtomList.add(atomIdentifierMap.get(i));
            }
        }
         System.out.println(carbonAromaticAtomList + " carbonAromaticAtomList" );
         System.out.println(numCarbAromatic + " carbonAromatic is present");
         AtomType atomName = AtomType.simple("C_Ar");
         if(numCarbAromatic >0){
             return 1;
         } else{
             return 0;
         }
    ...
    public static Map<String, double[]> atomicPotMap(){
        ...
        /*To print Values
        for(Map.Entry<String, double[]> entry : atomicPot.entrySet()){
...
*/
    ...
    public static void atomDistanceCalculator(){
        System.out.println("Distance Calculator ");
        for(int i =0; i<listOfBonds.size(); i++){
            int atom1 = listOfBonds.get(i).get(0);
            int atom2 = listOfBonds.get(i).get(1);
            double distance = distanceCalculator(atom1,atom2);
            System.out.println(atom1 + " "+ atom2+" "+distance);
        }
    }

    public static double distanceCalculator(int atom1, int atom2){
        List vector1 = locationList.get(atom1);
        List vector2 = locationList.get(atom2);
        double sum = 0.0;

        for (int i = 0; i < vector1.size(); i++) {
            double diff =  (double) vector1.get(i) - (double)vector2.get(i);
            sum += diff * diff;
        }
    ...
    public static double [] atomicPot (String atomtype){
        ...
        atomicConstant.put("C_2", new double[]{0.729, 120.0, 3.851, 0.105, 12.73, 1.912, 5.343, 2.0}); //C_Ar
        atomicConstant.put("C_3", new double[]{0.757, 109.47, 3.851, 0.105, 12.73, 1.912, 5.343,2.0});
        atomicConstant.put("C_Ar", new double[]{0.732, 120.0, 3.851, 0.105, 12.73, 1.912, 5.343,2.0});
    ...
    public static void main(String[] args) {
        //String confName = "F:/CH4";
        //speciesNew =getSpeciesNew(confName);

        ISpecies species= getSpeciesNew("F:/molecule"); //file location
        IMolecule molecule = species.makeMolecule();
        for(IAtom atom: molecule.getChildList()){
        ...
        for (int i=0; i<atoms.size(); i++) {
            ...
            for (int j=i+1; j<atoms.size(); j++) {
                ...
                System.out.println(a1+" "+" "+a1.getType()+" "+a2+" "+a2.getType()+"VDW "+a1.getLeafIndex()+" "+a2.getLeafIndex());
            }
        }
