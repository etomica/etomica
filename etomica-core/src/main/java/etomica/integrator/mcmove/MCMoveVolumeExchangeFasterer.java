 ...
 * @author David Kofke
 */
public class MCMoveVolumeExchangeFasterer extends MCMoveStep {

    protected final Box firstBox;
    protected final Box secondBox;
    private final IntegratorBoxFasterer integrator1;
    private final IntegratorBoxFasterer integrator2;
    ...
    public MCMoveVolumeExchangeFasterer(IRandom random,
                                        Space space,
                                        IntegratorBoxFasterer integrator1,
                                        IntegratorBoxFasterer integrator2) {
        super(null, new MCMoveStepTracker());
        this.random = random;
        ROOT = 1.0 / space.D();
    ...
    public void acceptNotify() {
        if (integrator1 instanceof IntegratorMCFasterer) {
            ((IntegratorMCFasterer) integrator1).notifyEnergyChange(uNew1 - uOld1);
        } else {
            integrator1.reset();
        }
        if (integrator2 instanceof IntegratorMCFasterer) {
            ((IntegratorMCFasterer) integrator2).notifyEnergyChange(uNew2 - uOld2);
        } else {
            integrator2.reset();
        }
    }
