...
public class MCMoveMoleculeExchangeFasterer extends MCMove {

    protected Box box1;
    protected Box box2;
    protected final IntegratorBoxFasterer integrator1, integrator2;
    ...
    private IntegratorBoxFasterer iIntegrator;
    private transient double uOld;
    private transient double uNew = Double.NaN;


    public MCMoveMoleculeExchangeFasterer(IRandom random,
                                          Space space,
                                          IntegratorBoxFasterer integrator1,
                                          IntegratorBoxFasterer integrator2) {
        super(null);
        this.random = random;
    ...
    public boolean doTrial() {
        IntegratorBoxFasterer dIntegrator;
...
//        dBox.removeMolecule(molecule);
    ...
    public void acceptNotify() {
        IntegratorBoxFasterer dIntegrator = integrator2;
        ...
        if (iIntegrator instanceof IntegratorMCFasterer) {
            ((IntegratorMCFasterer) iIntegrator).notifyEnergyChange(uNew);
        } else {
            //XXX grossly inefficient
            iIntegrator.reset();
        }
        if (dIntegrator instanceof IntegratorMCFasterer) {
            ((IntegratorMCFasterer) dIntegrator).notifyEnergyChange(-uOld);
        } else {
            //XXX grossly inefficient
            dIntegrator.reset();
        }
