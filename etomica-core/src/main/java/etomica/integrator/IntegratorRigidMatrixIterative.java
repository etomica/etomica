...
//                    System.out.println("force: "+((MyAgent)a.ia).force.toString());
            ...
            velocity.ME(momentum);
            KE += velocity.squared() * ((SpeciesGeneral)molecule.getType()).getMass();
...
//            System.out.println("rerandomize 1");
            ...
            Vector velocity = orientedMolecule.getVelocity();
            double mass = ((SpeciesGeneral)((IMolecule)atom).getType()).getMass();
            Vector moment = ((SpeciesGeneral)((IMolecule)atom).getType()).getMomentOfInertia();
            int D = velocity.getD();
            for(int i=0; i<D; i++) {
                velocity.setX(i,random.nextGaussian());
            }
