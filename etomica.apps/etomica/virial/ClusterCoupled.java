...
//        v /= (1 + r2*r2);
    ...
    private double invert(CoordinatePairSet cPairs) {
        ...
        for (int i=1; i<atomList.size(); i++) {
            ...
            double r = atom.coord.position().squared(); // sqrt
            if (r == 1.0) {
                return 1.0;
            }
            if (r > 1.0) {
...
//        System.out.println("now at "+minMaxAtom.coord.position()+" (r="+rN+")");
    ...
    public void setPhase(PhaseCluster phase) {
        atomList = ((AtomTreeNodeGroup)((AtomTreeNodeGroup)phase.getSpeciesMaster().getNode()).childList.get(0).getNode()).childList;
    }
