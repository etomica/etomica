...
//            System.out.println(selectedAtoms[i]+" "+j+" before "+selectedAtoms[i].coord.position());
            Vector position = selectedAtoms[i].getCoord().position();
...
//                System.out.println("end"+j+" move");
                ...
                work1.E(position);
                if (j == 0) {
                    work1.ME(((AtomLeaf)childList.get(j+1)).getCoord().position());
                    position.E(((AtomLeaf)childList.get(j+1)).getCoord().position());
                }
                else {
                    work1.ME(((AtomLeaf)childList.get(j-1)).getCoord().position());
                    position.E(((AtomLeaf)childList.get(j-1)).getCoord().position());
...
//                System.out.println("middle move "+j);
                Vector position0 = ((AtomLeaf)childList.get(j-1)).getCoord().position();
...
//                    System.out.println(i+" after "+k+" "+((AtomLeaf)childList.get(k)).coord.position());
                    if (k > 0) {
                        work2.E(((AtomLeaf)childList.get(k)).getCoord().position());
                        work2.ME(((AtomLeaf)childList.get(k-1)).getCoord().position());
...
//        System.out.println(uOld+" => "+uNew+"   "+wOld+" => "+wNew+" "+stepSize);
    ...
    public void rejectNotify() {
        for(int i=0; i<selectedMolecules.length; i++) {
            selectedAtoms[i].getCoord().position().ME(translationVectors[i]);
