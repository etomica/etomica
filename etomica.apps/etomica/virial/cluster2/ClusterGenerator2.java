 ...
 * @author andrew
 */
public class ClusterGenerator2 implements java.io.Serializable {

  public ClusterGenerator2(ClusterDiagram2 aCluster) {
    ...
    mConnectList = new int[nConnections][2]; // 4*N(N-1)*2 = 8N(N-1) bytes PER GRAPH!
  ...
  /**
   * Set whether to generate all permutations. If false, only one cluster from each set of permutations will
   * be generated. That cluster has the greatest "score" among its set of permutations, and the number of
   * identical permutations will be calculated and stored in the mNumIdenticalPermutations field.
   */
  ...
  /**
   * Set whether to generate Ree-Hoover diagrams. If set, then connected points have an f-bond and all others
   * have an e-bond.
   */
  public void setMakeReeHover(boolean flag) {
    ...
    mReeHoover = flag;
    if (flag) {
      mClusterCopy = new ClusterDiagram2(mCluster);
      mReeHooverGenerator = new ClusterGenerator2(mClusterCopy);
      mReeHooverGenerator.setAllPermutations(true);
      mReeHooverGenerator.setOnlyDoublyConnected(true);
    }
  }

  public static boolean swapAllAndCompare(boolean excludeRootPermutations, final ClusterDiagram2 cluster,
      int[] score) {
  ...
  /**
   * Determines whether any cluster permuted from the given one by exchanging points from startPoint to
   * stopPoint (inclusive) has a higher score than that passed in. If such a cluster is located, the method
   * returns true immediately. The method always restores the connections to their state when the method was
   * called. If the permutations of the given cluster do not have a higher score, the cluster's
   * mNumIdenticalPermutations field is updated to include the number of identical permutations encountered
   * while swapping.
   */
  private static boolean swapAllAndCompare(int startPoint, int stopPoint, boolean excludeRootPermutations,
      final ClusterDiagram2 cluster, int[] score) {
    ...
    if (startPoint + 1 < stopPoint
        && swapAllAndCompare(startPoint + 1, stopPoint, excludeRootPermutations, cluster, score)) {
      return true;
    }
   ...
   * while swapping.
   */
  public static void findMaxScore(boolean excludeRootPermutations, final ClusterDiagram2 cluster, int[] score) {
  ...
  /**
   * Determines if the cluster is connected after removal of the given node.
   */
  ...
  /**
   * Does the real work for the public is*Connected methods. The method walks through the cluster ensuring
   * that every point is reachable.
   */
  ...
  public boolean isElementary() {
    ...
    for (int i = mCluster.mNumBody - 1; i >= 0; i--) {
      if (isNodalPoint(i) || isArticulationPoint(i))
        return false;
      for (int j = i - 1; j >= 0; j--) {
  ...
  /**
   * Nodal Point: traverse all connected components starting from field nodes only;
   * if any component contains all root nodes, then the node is a nodal point.
   */
  public boolean isNodalPoint(int exclude) {
    ...
    // the stack starts with one FIELD NODE; we traverse the graph
    // and guarantee that we don't find all root nodes in the same
    // connected component
    while (stackLength > 0) {
      // pop the top node from the stack
      thisNode = mStack[stackLength - 1];
      stackLength--;
      // traverse the neighbors of thisNode
      for (i = 0;; i++) {
        // get the next neighbor of thisNode
        connectedNode = mCluster.mConnections[thisNode][i];
        if (connectedNode == -1) {
          // end of connections for thisNode-
          // break and proceed with the next
          // node on the stack
          break;
        }
        // found an unvisited node
        if (!mMarked[connectedNode]) {
          // if the unvisited node is a root node, increment the
          // count of root nodes found thus far and check if we
          // found them all
          ...
          // if we haven't found them all yet, mark the node as
  ...
  /**
   * Method that does the real work for the isArticulation* methods. The method walks through the cluster
   * looking for root nodes. If it finds a root point in every section, it returns false. If all root points
   * were deleted in calling methods, then it returns false if all filled points are in one section.
   */
      ...
      while (stackLength > 0) {
        ...
        for (int i = 0;; i++) {
          ...
          if (!mMarked[connectedNode]) {
            if (!mCluster.isRootPoint(connectedNode)) {
              numMarked++;
              if (numMarked == numMarksNeeded && reachedReferenceNode) {
                return false;
              }
            }
            else {
  ...
  /**
   * Determines if the cluster has a maximum score. If the score can be increased by swapping filled points or
   * by swapping reference nodes, then one of its permutations must have a higher score and the method returns
   * false. If the cluster does have a maximum score, the mNumIdenticalPermutations is calculated as 1 + the
   * number of permuted clusters with the same score as the unpermuted cluster.
   */
  ...
  /**
   * Calculates the Ree-Hoover coefficient (coefficient for Ree-Hoover diagram / coefficient for Mayer
   * diagram) for mCluster and sets mCluster's mReeHooverFactor to it.
   */
  ...
  /**
   * Resets the cluster generator. This resets the cluster, and prepares the generator to generate clusters
   * matching the appropriate criteria.
   */
  public void reset() {
  ...
  /**
   * Advances the cluster to the next cluster matching the given criteria. Returns false if no more clusters
   * exist.
   */
  public boolean advance() {
    ...
    for (; mCurrentConnection < mTotalNumConnections || mConnectStackLen > 0; mCurrentConnection++) {
      ...
      if (!mCluster.deleteConnection(node1, node2)) {
        throw new RuntimeException("failed to delete a connection that should have existed");
      }
  ...
  public static void main(String[] args) {
    ...
    if (args.length > 1)
      nRootPoints = Integer.parseInt(args[1]);
    ClusterDiagram2 cluster = new ClusterDiagram2(nBody, nRootPoints);
    ClusterGenerator2 generator = new ClusterGenerator2(cluster);
  ...
  private ClusterDiagram2 mClusterCopy;
  ...
  public final ClusterDiagram2 mCluster;
  private ClusterGenerator2 mReeHooverGenerator;
